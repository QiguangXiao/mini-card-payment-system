...
class SimulatedNotificationProviderControllerTest {
    ...
    void sameProviderAndIdempotencyKeyReturnsSameMessageId() {
        ...
        NotificationProviderClient.NotificationProviderResponse first = controller.send(request);
        NotificationProviderClient.NotificationProviderResponse duplicate = controller.send(request);
    ...
    void differentProvidersDoNotDeduplicateEachOther() {
        NotificationProviderClient.NotificationProviderResponse push =
                controller.send(request("push", NotificationChannel.APP_PUSH, "delivery-1"));
        NotificationProviderClient.NotificationProviderResponse email =
    ...
    private NotificationProviderClient.NotificationProviderRequest request(
            String providerName,
            NotificationChannel channel,
            String idempotencyKey
    ) {
        return new NotificationProviderClient.NotificationProviderRequest(
                providerName,
                channel,
                "recipient-address",
                "title",
