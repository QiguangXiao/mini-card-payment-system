 ...
 * 1. SELECT authorization FOR UPDATE（业务表才是 source of truth，job 只是执行计划）；
 *    非 APPROVED: 幂等 skip（已 posted/expired，retry/手工重放安全）；
 *    now 仍早于 expiresAt: 抛异常（job 早跑是 bug，不能提前释放）
 * 2. load card（不锁），拿到 creditAccountId
 * 3. SELECT credit_account FOR UPDATE（与新授权 reserve 串行化）
 * 4. account.release(reserved amount)；authorization APPROVED 变 EXPIRED
 * 5. append Outbox event authorization.expired，COMMIT（release/EXPIRED/event 三件事同事务）
 * </pre>
 */
@Service
@Slf4j
