...
- 通知渠道：每渠道独立断路器 + TimeLimiter + slow-call 阈值，"又慢又错"的 provider 被快速隔离。
...
- **扣分项**：仓库卫生（`bin/`、`build/`、`.idea/` 入库）；两个真实缺陷
  （OVERDUE 不可达、自动扣款竞态，见 §6）如果被读代码的面试官先发现，会反噬"严谨"人设。
...
### Q12.（若面试官读代码）"statement 什么时候变 OVERDUE？"
...
### 6.1 `OVERDUE` 状态不可达 — 严重度 High

**现状**：`StatementStatus.OVERDUE` 只被两处引用——`Statement.applyRepayment` 的
...
| 部分 | 问题 | 建议 |
| --- | --- | --- |
| 通知投递栈（每渠道断路器 + TimeLimiter + lease 状态机 + 故障注入） | 全项目最大子系统之一，但与 issuer 核心域无关 | 保留代码；面试降为一句话："通知走同一套可认领框架，附带渠道级熔断" |
...
| 2 | 修 OVERDUE（§6.1）+ 自动扣款竞态（§6.2） | 各半天~一天 | 消灭两个致命追问点，同时生产两个最佳面试故事 |
...
> slow-call detection and a TimeLimiter — so a failing push provider can't drag email down.
