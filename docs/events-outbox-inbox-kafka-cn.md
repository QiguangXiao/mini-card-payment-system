...
- **有序性**：partitionKey=聚合 id ⇒ 单聚合有序。`concurrency>1` 不破坏单 partition 顺序（同 group 多线程各吃不同 partition）。**DLT 会局部乱序**：某条重试耗尽进 DLT 后，同 key 后续继续；若是"按序推导余额"的有状态投影，就要靠 DLT replay/补偿或暂停该 key，不能假设失败记录会自动补回原顺序。
...
**Q6. 顺序性怎么保证？什么时候乱序？**
partitionKey=聚合 id 保证同聚合有序；`concurrency=3` 不破坏单 partition 顺序（各吃不同 partition）。DLT 会局部乱序（同 key 跳过失败那条继续）；跨 topic 无全局有序，consumer correctness 不能依赖跨 topic 顺序。
