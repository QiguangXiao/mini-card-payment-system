...
这份文档把 JVM 基础、GC、内存增长模拟、线上排查和面试回答放在一起讲，并尽量绑定到本工程的真实类、请求路径、线程池和 Actuator 配置。

当前工程背景：

- Java 21, Spring Boot 3.5.14, Spring MVC/Tomcat, MyBatis, MySQL, Kafka, Actuator。
- 主要业务链路包括 authorization、presentment posting、statement、repayment、notification、risk、ledger。
...
- 原始采集文件保存在 `build/diagnostics/idea-jvm-2026-06-21-1243/`，完整 thread dump 是 `thread-dump-after-authorization.txt`。
...
- `outboxTaskScheduler = 1`、`delayJobTaskScheduler = 2`、`statementBatchTaskScheduler = 1`。
...
- 当前没有发现阻断 authorization、Outbox、Consumer Inbox、notification 或 risk projection 的缺失字段、缺失索引、缺失唯一约束或缺失 check constraint。
...
- 管理 thread stack：Tomcat 请求线程、scheduler 线程、Outbox/DelayJob worker 线程、Kafka listener 线程。
...
- `delay-job-worker-*`：处理授权过期、自动还款等未来业务动作。
- `statement-batch-scheduler-*`：触发账单批处理。
- Kafka listener thread：Notification、Risk feature、Ledger consumer。
...
- inbox row、notification/ledger/risk projection object
...
concurrency 提高可以提高处理能力，但也会提高同时活跃的线程、stack、record buffer、DB 连接需求。面试里要强调：consumer concurrency 不能超过 DB、CPU、partition 数和幂等写入能力的整体瓶颈。
...
`HealthController` 位于 `monitoring.api`，它只提供轻量 liveness：
...
这个 endpoint 不查 MySQL，不查 Kafka，也不计算 JVM 内存。

原因：

- public liveness 只回答“HTTP 应用是否还活着”。
- DB 抖动不应该导致进程被错误重启。
- JVM、GC、thread、dependency health 属于 management plane，交给 Actuator/Micrometer。
...
- Statement batch 不能一次性把所有账户和交易塞进内存，应继续使用 `max-accounts-per-run` 和按账户事务处理。
...
- `outbox-worker-*` 固定 4。
- `delay-job-worker-*` 固定 4。
- scheduler pool 固定。
- Kafka listener concurrency 固定。
...
本项目已经把风控放在 account row lock 之前，这是为了缩短 critical section。
...
- Notification 是否延迟。
- Risk feature projection 是否落后。
- Ledger projection 是否落后。
...
- Risk external timeout / circuit breaker state
...
- 是单实例还是所有实例？
- 是所有 API 还是只有 authorization / statement / repayment？
...
- Worker queue depth、rejected count。
