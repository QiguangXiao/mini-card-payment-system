...
| configuration properties | `AuthorizationPolicyProperties`、历史版本的 `SnapshotCacheProperties` | YAML 绑定为 typed record | `@Value` 分散，类型/默认值难统一管理 |
...
| Caffeine + Redis | 历史版本的 `SnapshotCacheFactory`、`TwoLevelSnapshotCache` | L1 降本机热点，L2 跨实例共享 | Redis 故障或热点 miss 容易放大成 API/DB 压力 |
...
## 7. MyBatis、JDBC 与 Liquibase

更完整的 MyBatis/SQL 说明见 `docs/mybatis-sql-learning-cn.md`。这里讲技术习惯层。

### 7.1 `@Mapper` 和 `@Param`

位置：`AuthorizationMapper`

`@Mapper` 让 MyBatis 为接口生成 runtime proxy。

`@Param` 给 XML 参数一个稳定名字：
...
- XML 里可能只能用 `param1`、`param2`。
- 可读性差。
- 参数顺序一改，SQL 可能悄悄错。

### 7.2 XML `namespace` 和 `id`
...
```text
namespace = mapper interface full name
id = method name
```
...
- 应用可能启动成功。
- 第一次调用 repository 时抛 `invalid bound statement`。
- 这种错误比编译错误晚，所以 XML 路径和名字需要格外稳定。
...
`#{}` 使用 prepared statement bind parameter。
...
只有列名、排序方向这类无法用 bind parameter 的 SQL 片段才可能用 `${}`，而且必须先做 whitelist。

### 7.4 constructor mapping + record row
...
constructor mapping 直接创建不可变 row record。

如果用 setter-based mutable row：

- MyBatis 映射后对象还可能被改。
- row 的完整性不如 constructor 明确。
- persistence adapter 和 domain mapping 更容易混杂。
...
SQL row
-> AuthorizationRow
-> MyBatisAuthorizationRepository.toDomain()
-> Authorization aggregate
```

这样 domain 不需要知道表字段、字符串 enum、MyBatis XML。

### 7.5 `Optional` 用在 repository 返回值
...
`Optional` 表达“没有找到是正常分支”。

如果返回 nullable：

- 调用方容易漏判。
- NPE 发生点离 repository 很远。
- review 时看不出缺失资源是否被处理。

注意：本项目不把 `Optional` 用作字段类型，主要用于方法返回值。

### 7.6 `DuplicateKeyException`

位置：`MyBatisAuthorizationRepository.claim`

Spring 会把数据库 duplicate key 错误转换成 `DuplicateKeyException`。

这比直接依赖 MySQL driver exception 更好：

- 代码不绑死具体数据库 driver。
- repository 只关心“唯一约束冲突”这个语义。
- 迁移或测试替换数据库时更稳定。

### 7.7 Liquibase 与 SQL init
...
本项目把 `spring.sql.init.mode` 设为 `never`，schema 交给 Liquibase。

原因：

```text
schema.sql is one-shot initialization
Liquibase is versioned migration
```

如果数据库 volume 已经存在旧表，`schema.sql` 不会帮你把旧结构迁移到新结构。

如果同时启用 SQL init 和 Liquibase：

- 两套 schema 来源可能互相打架。
- 本地数据库和代码 migration 历史容易漂移。

## 8. Kafka、Spring Kafka 与 Jackson

更完整 Kafka 说明见 `docs/kafka-learning-cn.md`。

### 8.1 `NewTopic` 和 `TopicBuilder`

位置：`KafkaMessagingConfiguration`

`NewTopic` bean 让 Spring Kafka admin 在启动时确保 topic 存在。

如果不声明：

- 依赖 broker 的 auto-create topic。
- partition/replica 数可能不是你想要的。
- 本地能跑，生产环境可能禁止 auto-create。

本项目设置 3 partitions 是为了演示 consumer group parallelism。

注意：

```text
partition count affects maximum parallelism inside one consumer group
replica count affects broker-side durability
local single broker can only replicas=1
```

### 8.2 `ConcurrentKafkaListenerContainerFactory`
...
本项目先调用 Boot configurer：

```java
configurer.configure(factory, consumerFactory);
```

再设置项目自己的 error handler 和 concurrency。

如果不调用 configurer：

- `application.yml` 中的 consumer/listener 默认配置可能没套进去。
- serializer/deserializer、ack mode 等行为不一致。

### 8.3 `DefaultErrorHandler` 和 DLT

位置：`KafkaMessagingConfiguration`

```text
listener throws exception
DefaultErrorHandler retries with FixedBackOff
still fails
DeadLetterPublishingRecoverer sends record to DLT
```

如果没有 DLT：

- 坏消息可能反复阻塞同一个 partition。
- 或者错误被吞掉，下游 side effect 丢失。
- replay 和排查没有稳定落点。

本项目把 `EventContractException` 标成 not retryable，因为 malformed JSON、版本不支持、header 不一致这类问题重试不会自愈。
...
- Ledger/Notification listeners

重要参数：

| 参数 | 含义 | 不正确时 |
| --- | --- | --- |
| `topics` | 订阅哪个 topic | 收不到或收到不该处理的事件 |
| `groupId` | 哪个 consumer group 的进度 | 多个 bounded context 互相抢消息 |
| `containerFactory` | retry/DLT/concurrency 策略 | 错误处理和并行度不符合上下文 |
...
它们都能看到同一条事件，但各自维护 offset。

如果错误地共用 group：

```text
Notification consumer takes partition
Risk consumer does not receive that record
Risk projection misses event
```

### 8.5 `KafkaTemplate`、`ProducerRecord` 和 headers

位置：`KafkaOutboxMessagePublisher`
...
- observability。
- contract validation。
- 不先反序列化 payload 也能读取 eventType/eventId。

`kafkaTemplate.send(record).get(timeout...)` 等待 broker acknowledgement。

如果不等 ack 就标记 Outbox 为 `PUBLISHED`：

```text
application marks row PUBLISHED
broker write actually failed
event is lost
```

等待 ack 仍然不是 exactly-once。应用在 Kafka ack 后、更新 Outbox row 前宕机，事件会再次发布。所以 consumer 仍要幂等。

### 8.6 捕获 `InterruptedException` 后恢复 interrupt flag
...
如果 catch 后不恢复 interrupt flag：

- 上层框架不知道线程被要求中断。
- shutdown 可能变慢。
- worker thread 可能继续执行本应停止的工作。

### 8.7 `ObjectMapper` 和 `JsonNode`
...
本项目的 event envelope 是稳定 record，payload 使用 `JsonNode`：
...
- envelope 统一。
- 不用为每个 event type 创建一组 payload class。
- consumer 只读取自己关心的字段。

代价：

- 失去 compile-time field check。
- consumer 必须显式调用 `requiredText`、`requiredInstant`。
- contract validation 要集中，否则错误行为不一致。

如果 payload 直接是 `Map<String, Object>`：

- 类型更弱。
- 时间、数字、UUID 转换更容易出错。
- Jackson tree API 的字段类型判断不如 `JsonNode` 清晰。

## 9. 缓存、Redis 与 Caffeine

当前代码里的 Redis 主要用于 `RedisRiskVelocityCounter` 的 sliding-window velocity 计数。
下面的 Caffeine L1 + Redis L2 内容是已删除 snapshot cache 的历史实现，用来学习
cache-aside、TTL jitter、single-flight 和 after-commit eviction。更完整的取舍说明见
`docs/distributed-cache-cn.md` 和 `docs/cache-snapshot-design-cn.md`。

### 9.1 为什么用 `StringRedisTemplate`

位置：历史版本的 `SnapshotCacheFactory`

本项目 Redis L2 存 JSON string：

```text
key: mini-card:cache:statement-read-model-v1:{id}
value: JSON
```

`StringRedisTemplate` 明确表达 key/value 都是字符串。

如果用默认 Java serialization：

- Redis 里是不可读二进制。
- Java class 改名或字段变化时反序列化更脆。
- 与非 Java 工具排查不友好。

JSON 的代价是需要处理 schema 演进，所以 cache name 带版本号。

### 9.2 复用 Spring Boot 的 `ObjectMapper`

Spring Boot 已经配置了 Jackson 对 Java time、record、HTTP JSON 的支持。

缓存层复用同一个 `ObjectMapper`：

- API JSON 和 cache JSON 行为一致。
- 不需要重复注册 JavaTimeModule。
- 未来统一配置序列化格式时更集中。

如果每个组件自己 `new ObjectMapper()`：

- Java time 可能序列化失败或格式不一致。
- 配置漂移。
- 测试和生产行为不同。
...
- L1 在 JVM heap 内。
- 如果无限增长，热点或恶意 key 会吃掉应用内存。

为什么 L1 TTL 比 Redis 短？

- 多实例之间 L1 不共享。
- 一个实例 evict Redis，不会自动 evict 其他实例的 L1。
- 短 TTL 降低 stale window。
...
- Redis read 失败：log warning，回源 MySQL。
- Redis write 失败：本次请求仍返回。
- Redis JSON 坏了：删掉坏值，回源 MySQL。

如果 Redis 异常直接抛给业务 GET：

```text
cache outage
-> statement/card query outage
-> cache becomes availability dependency
```
...
注意：它只解决单 JVM。跨实例还需要 TTL jitter、Redis TTL，未来超热点可再加 distributed mutex。
...
如果所有 Redis key 都是精确 5 分钟：

```text
batch writes many keys at 10:00:00
all expire at 10:05:00
next request wave hits DB together
cache avalanche
```

`ThreadLocalRandom` 适合多线程热点路径，避免共享 `Random` 带来的竞争。
...
| Statement | 已补强 | controller/scheduler 双入口、`YearMonth`、private record、business calendar、历史 snapshot cache、repository duplicate 粒度 |
...
| Cache | 历史设计已覆盖 | Caffeine L1、Redis L2、`StringRedisTemplate`、TTL jitter、single-flight、after-commit evict |
...
7. 读 `docs/cache-snapshot-design-cn.md` 的历史 cache 设计，理解第三方 cache 和 transaction hook；当前代码可读 `RedisRiskVelocityCounter` 理解 Redis 的实际落脚点。
...
11. 读 `StatementBatchService`、`JapaneseBusinessDayCalendar`、`StatementService`，理解 `YearMonth`、private record、`Set.copyOf`、`getFirst()` 和 rounding。
