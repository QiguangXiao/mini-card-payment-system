...
| Notification | `StatementNotificationListener` | statement topic | `statement.closed` | 创建 `STATEMENT_CLOSED` 通知 | 同上 | 同上 |
...
-> notifications(STATEMENT_CLOSED)
```
...
| 省掉 Outbox 后的情况 | 结果 |
| --- | --- |
| MySQL commit 成功，Kafka publish 失败 | 用户授权/还款已经成功，但 Notification/Ledger/Risk 收不到事件 |
...
- 通知会重复发送。
- Ledger 会多记一笔 entry。
- Risk feature 会重复累计，下一笔授权可能被误拒绝。
...
- `statement.closed` 是 integration event（账单事实）。
- `STATEMENT_CLOSED` 是 Notification context 里的通知类型；与事件同名，保持四类通知 1:1 命名一致。
