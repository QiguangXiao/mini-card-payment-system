...
- **单列 lease**：`next_attempt_at` 一列三用——PENDING 时=最早可执行时间，PROCESSING 时=lease 截止 + finalize token。
- **指数退避**：`markFailed` → `min(2^(n-1), 60s)`。
- 派发表启动时由 `DelayJobHandler` 列表转 `EnumMap`；缺 handler 走失败路径而非静默 DONE。

> 更正一个常见误解：`delayjob` **也有** lease token 校验（`DelayJobWorker.lockCurrentLease` 比较 `nextAttemptAt`）。三家族区别不在"有没有 lease 校验"，而在粒度/退避/lease 列模型。
...
- 单列 lease（同样复用 `next_attempt_at`），指数退避对称，终态是 **`PUBLISHED`**（不是 DONE）。
...
- **多列 lease（更丰富）**：`claimed_by / claimed_at / claim_until`；`claim_until` 作 lease 截止 + finalize token。比 delay/outbox 多了"谁在处理、何时开始"的运维可见性。
...
claimed_by / claimed_at / claim_until               ← lease（claim_until 兼 lease token）
attempt_count
processed_/generated_/skipped_/failed_account_count ← 分片级计数（observability）
created_at / updated_at / last_error
```

约束：`uk(period_start, period_end, shard_no)`（创建幂等）；`idx(status, claim_until, created_at)`（claim/recover 扫描）；`chk` 保证 PROCESSING 必有完整 lease、`period_end >= period_start AND due_date > period_end`。
...
每 1s  StatementJobDispatcher.dispatch：短事务 FOR UPDATE SKIP LOCKED 领 PENDING
       → 改 PROCESSING（写 claimed_by/claim_until）→ commit → 交 statementJobWorkerExecutor
worker  StatementJobHandler.handle：CRC32(account)%shardCount 取本片账户，逐个 generate（各开小事务）
finalize 新短事务：findByIdForUpdate 重锁 job + 校验 claim_until 仍是自己 → 标 DONE / PENDING / DEAD
...
- **Optimistic lease token**：finalize 前比对令牌（复用 `next_attempt_at` / `claim_until`），防迟到 worker 覆盖新状态。
...
**Q2. PROCESSING lease 是什么？为什么 delay/outbox 用单列 `next_attempt_at`，statement 用多列 `claim_until`？**
lease 是"限时处理权"。delay/outbox 一个 job 是单一工作单元，复用 `next_attempt_at`（PENDING=下次执行时间，PROCESSING=租约截止+令牌）够省；statement 是分片 fan-out，需要 `claimed_by/claimed_at` 的运维可见性。
- *追问：时间戳令牌，两 worker 同一 `now` claim 同一行会撞吗？* 不会。claim 在短事务 `FOR UPDATE SKIP LOCKED` 对同行串行化，只一个 winner 写 lease；令牌按行比较。
