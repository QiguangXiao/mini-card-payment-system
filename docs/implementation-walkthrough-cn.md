...
`messaging/outbox` 和 `delayjob`，而 statement 出账已扁平化为 claimable job（`BillingCycleScheduler` 每日触发 + `StatementJobDispatcher` 的 claim/recover）。
...
4. `creditAccountRepository.findByIdForUpdate(accountId)`
   ...
   因为 `StatementGenerationService` 也先锁账户，再锁待出账交易，统一锁顺序可以避免 posting/statement 死锁，
   同时防止账单生成期间漏掉正在入账的交易。
...
- 一个账户失败不连坐整批（账户级计数隔离）。
- 每账户独立 transaction boundary。
- 行锁持有时间短，便于横向扩展。分片/扇出/lease/recover 的完整机制见 `claimable-jobs-cn.md`。

`StatementJobDispatcher` 的 lease 现在拆成两个维度：

| 维度 | 字段 |
| --- | --- |
| lease deadline | `claim_until` |
| finalize owner check | `claim_token` |
| 可观测字段 | `claimed_by / claimed_at / claim_until` |

`claim_until` 回答“worker 多久没回来就可以 recover”；
...
```
   ...
   这样 repayment 不会和 `StatementGenerationService.generate(...)` 的锁顺序相反，降低死锁风险。
...
> 本节是 Outbox 在主流程里的走读位置；其完整机制（dual-write、claim/publish/finalize/recover、at-least-once、消费者双层幂等、DLT）见 `events-outbox-inbox-kafka-cn.md`。
...
> 本节是 DelayJob 在主流程里的走读位置；DelayJob 与 Outbox/StatementJob 三大 claimable-job 家族的对比、lease/recover/退避机制见 `claimable-jobs-cn.md`。
...
> 这部分已并入 `claimable-jobs-cn.md` 的三家族总对比表（DelayJob/Outbox/StatementJob），不在此重复。
...
当前阶段已经有 `CardTransaction`、`Statement` 和 `Repayment`：

- `CardTransaction` 提供 posted transaction 基础。
- `Statement` 把 posted transactions 固定成账单快照。
- `Repayment` 处理还款、恢复额度和账单结清。

本项目不实现 Ledger：生产 accounting 需要 balanced journal、会计科目、调整/冲正和严格审计，
不应使用只有单边方向标签的异步 projection 冒充 source of truth。
...
> Spring 注解/DI/事务/配置/Scheduler 用法见 `spring-java-technical-learning-cn.md`；MyBatis mapper/动态 SQL/锁/索引见 `mybatis-sql-and-migration-cn.md`。
...
## 14. 授权与账户聚合设计决策（合并自 authorization-design）

> 本节合并自原 `authorization-design.md`（英文）。前面是 request-to-table 的走读，这里补"为什么这样设计"的聚合边界与不变量视角。

### 14.1 Aggregate boundary 与 invariants

- **`Authorization` 是 aggregate root**：外部代码不能直接赋 status，只能经 `approve`/`decline`/`post`/`expire` 行为推进。保护的不变量：新建为 `PENDING`；只有 pending 能 approve/decline；approved 有 decision time、无 decline reason；posted 必须先 approved 且在 `expiresAt` 前；declined 同时有 decision time 和 decline reason；expired 必须先 approved 且在 `expiresAt` 当时或之后；pending 既无 decision time 也无 decline reason。`Money` 是 value object，把 amount 和 currency 绑在一起、在落库前拒绝非法金额。
- **`CreditAccount` 是独立 aggregate root**（额度/reserved 有自己的生命周期与一致性规则），不内嵌进 Authorization；授权决策**不需要** user/customer aggregate。账户不变量：reserved/posted 非负；`reserved + posted ≤ creditLimit`；limit/reserved/posted/请求金额同币种；blocked 账户不能 reserve；reservation 不超当前 available。`available = creditLimit - reservedAmount - postedBalance`。仓库用 `SELECT ... FOR UPDATE` 加载账户，同账户并发授权在检查/改 available 前被串行化，防超额。
- **`Card` 聚合**把卡标识映射到 CreditAccount、拥有卡生命周期；支持多卡共享一个账户。授权流程在锁/改 CreditAccount **之前**拒绝 missing/blocked/expired 卡。`CardRepository` 现直接经 MyBatis 读卡参考数据；旧的 `CachedCardRepository`（`CardSnapshot` + Caffeine L1 + Redis L2）已移除，因为 stale card status 会影响授权批准——即使未来缓存卡查询，也**绝不**缓存 CreditAccount、available credit 或 idempotency ownership，金融一致性边界永远是 MySQL 事务 + CreditAccount row lock。`authorizations` 故意保留 `card_id` 而**不**对 `cards` 加外键（这样能保留对未知卡的 declined 记录）；生产里 `cardId` 应是内部 token，绝不是明文 PAN。

### 14.2 Idempotency、并发与持久化防御

- **Idempotency key 是 request-processing metadata**，存在行上提供原子的数据库唯一边界；行还存 SHA-256 request fingerprint，用来拒绝同一个 key 复用于不同 card/amount/currency/merchant/geolocation。MySQL 经唯一约束选出一个 winner（拥有 pending Authorization、可 reserve credit）；duplicate 阻塞在 winner 之后，再用 `SELECT ... FOR UPDATE` 读已完成结果（在 MySQL 默认 `REPEATABLE READ` 下避免重复 reserve 和 stale 快照）。
- **统一锁顺序**：`Authorization idempotency row -> Card read -> CreditAccount row`，随着流程增多降低死锁风险。同 key 同请求返回原结果；同 key 不同请求返回 `409 Conflict`。approved/declined 都返回 HTTP `200`（decline 是有效业务决策，不是技术失败）。
- **Persistence defense**：domain 是业务规则的主要场所，但 MySQL 表也加约束（正数金额、有效 status/decision 列组合、账户 `reserved+posted ≤ limit`）保护非正常路径写入。主持久化用 MyBatis XML mapper，domain repository 接口独立于 MyBatis（`domain Repository interface ← MyBatis adapter ← Mapper + XML + Row`）；`JdbcRiskVelocityCounter` 保留为 `RiskVelocityCounter` port 背后的 JdbcTemplate 对照示例。

### 14.3 刻意推迟的生产关切

生产风控模型与真实 provider 合约、presentment 前的网络授权撤销、partial presentment / partial hold 释放、refund / clearing adjustment / dispute / chargeback、生产级 double-entry ledger 与 reconciliation 报表、后续聚合更新的乐观版本控制、user/cardholder 身份与认证授权与 PII 处理、生产级迁移部署控制（在线 DDL review、灰度窗口、forward-fix playbook）。
