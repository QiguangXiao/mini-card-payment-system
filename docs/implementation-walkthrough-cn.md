...
`messaging/outbox` 和 `delayjob`，而 `BillingCycleScheduler` / `StatementJobDispatcher`
是 statement 模块自己的日历触发与 job claim 回路。
...
-> statement_jobs/PENDING
-> StatementJobDispatcher.dispatch()
...
- `BillingCycleScheduler.createDueStatementJobs()`
- `StatementCycleService.createDueJobs(runDate)`
- `StatementBillingRepository.countBillableAccounts(...)`
- `StatementJobDispatcher.dispatch()`
- `StatementJobHandler.handle(job)`
...
### 5.1 Cycle / Job 和 Command 阶段

`BillingCycleScheduler` 是很薄的 calendar scheduler：

- 用 `@Scheduled` 每天按 `Asia/Tokyo` 醒一次。
- 只负责触发“是否需要创建本期 statement jobs”的判断。
- 不在 scheduler 方法里持有大事务。
- 不直接拼 SQL 锁业务表。

`StatementCycleService` 负责：

- 根据 `statement.batch.close-day-of-month` 判断昨天是否是关账日。
- 计算 `periodStart / periodEnd / dueDate`。
- 统计有未出账 posted transactions 的账户数量。
- 按 `targetAccountsPerJob` 计算 shard 数，并用
  `uk_statement_jobs_cycle_shard(period_start, period_end, shard_no)` + `INSERT IGNORE`
  幂等创建 `statement_jobs/PENDING`。

这里刻意不是一个大事务：

- 一个账户失败不能拖垮整批。
- `StatementJobDispatcher` 只在短事务里 claim job；真正生成账单交给 worker pool。
- `StatementJobHandler` 再把一个 shard 拆成一个个账户，每个账户自己的 statement generation
  都有清楚的 transaction boundary。
- 行锁持有时间短，便于未来横向扩展 job worker。

`StatementJobDispatcher` 的 lease 改前/改后：

| 维度 | 改前 | 改后 |
| --- | --- | --- |
| lease deadline | `claim_until` | `claim_until` |
| finalize owner check | 比较 `claim_until` | 比较 `claim_token` |
| 可观测字段 | `claimed_by / claimed_at / claim_until` | 保持这些字段，并新增 `claim_token` |

`claim_until` 现在只回答“worker 多久没回来就可以 recover”；
`claim_token` 才回答“这个迟到 finalize 是否仍属于本次 claim”。
...
声明事务边界。
当前最关键的是 `AuthorizationService.authorize()`、`PostingService.post()`、`StatementGenerationService.generate()`、`RepaymentService.receive()` 和 `AuthorizationExpiryService.expire()`。
