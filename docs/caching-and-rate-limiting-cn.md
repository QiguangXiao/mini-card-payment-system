...
cache name = `statement-read-model-v2`（带版本号，见 §2.6）。配置前缀 `statement.read-cache.*`：
...
Redis value 是 JSON contract。字段语义不兼容时，**改 cache name（`v1`→`v2`）比清全量 Redis 更可控**，旧 key 自然过期。本项目这次就因为 L2 value 从"纯 JSON"变成"`<version>|<payload>`信封"（见 §3.3）而把名字 bump 到了 `v2`。
...
`after-commit delete` 关不掉 cache-aside 固有的"迟到写"。约定：`version = paidAmount 的最小货币单位`（本域单调非减，新关账 paid=0→v0，还款到 ¥100→v100）。

**两条 race（很多人只看到一条）：**

```text
Race A（慢旧 reader 覆盖已存新值）：
  t0 GET-A 读 DB paid=0，写 L2 很慢
  t1 还款 commit，evict
  t2 GET-B 读 DB paid=100 → 写 L2 {100}     ← L2 已是新值
  t3 GET-A 迟到写 {0}                         ← 覆盖新值！
...
4. **version 完整性耦合**：若将来加"改 read model 却不改 paidAmount"的字段，同版本不同值会被 CAS 误判，需换专用 version 列。
...
单纯 after-commit delete 关不掉：delete 把可比版本也删了，迟到写落在空 key 上（Race B）。我用"版本号 + Lua CAS + 墓碑"：evict 不 delete，而写一个 `version=新paidAmount` 的墓碑做地板；读回填和墓碑写共用一段 CAS（`stored.version > incoming 则拒绝`），旧版本迟到写被挡下。
...
更新缓存并发下可能写旧计算结果，且写侧要会拼完整 read model → 两条构造路径会发散；删/失效让"只有读路径构造缓存值"。强一致不靠 cache：`paid_amount/status` 由 `credit_accounts` 行锁 + DB 事务保证，要强一致就直接读 DB，缓存只在 GET 展示路径提速，秒级 stale 已知且可接受。
