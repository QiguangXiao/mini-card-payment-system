...
本文档是 PayPay Card 面试的**全类型**问答种子库：覆盖专业基础（CS 基础 + 领域核心）、
通用后端工程、全新系统设计题、行为面试、团队/心理/人际、国际化与动机、逆质问。
与现有文档的分工：
...
四方模型（4-party scheme）的参与者：持卡人（cardholder）、加盟店（merchant）、
加盟店收单方（acquirer）、发卡行（issuer），国际品牌（Visa/Mastercard 等 network）居中做
清分结算和规则制定。用一笔 10,000 円购物模拟钱流：

1. **授权时刻**：不动钱。issuer 只是冻结持卡人额度（本项目里的 reserved amount），
   给 network 返回 approve。
2. **清分（clearing）**：加盟店经 acquirer 向 network 提交请款记录，network 生成清分文件
   分发给 issuer——这对应本项目的 presentment posting。
3. **结算（settlement）**：issuer 把约 10,000 円 −（interchange fee）付给 network，
...
2. **Interchange fee**：每笔消费从加盟店手数料中分到的部分。日本 2022 年起
   Visa/Mastercard 公开了标准 interchange 料率，平均约 2.3%（锚点数字，面试前核对）。
...
1. **Authorization（授权）**：冻结额度，不动钱。同步低延迟路径
   （network 要求秒级应答），幂等靠 `Idempotency-Key` + 唯一约束，
   并发安全靠账户行锁。不这样做：客户端重试重复占额度、并发刷卡超限额。
...
3. **Statement（出账）**：按账单周期把 posted 交易固化成账单，
   `statement_lines` 保存快照，账单稳定、可审计。不固化快照：
...
（速Q1–速Q6 与 Q8–Q15），这里的版本用于面试开场介绍项目时的领域铺垫。
...
两条路径的 SLA、幂等键、失败处理完全不同，这正是我项目里把
...
1. **DNS 解析**：本地缓存 → 系统 resolver → 上游解析；K8s/服务发现环境里
   这一步换成集群 DNS 或注册中心。
...
**概念先纠偏**：load average 统计的是「可运行 + 不可中断睡眠（D 状态，
...
4. **顺手全查**：`df -h` 与 `df -i`（磁盘满、inode 满）、`free -m`
...
防御两条：fd 使用率进常态监控（70% 报警，等打满就是事故了）；
...
线程在这段时间只能干等——并发能力 = 线程数。这就是两个模型的分水岭：
**请求处理是否阻塞**决定要不要靠线程数堆并发。出路两条：虚拟线程把
阻塞变便宜（E6），或响应式把阻塞消掉（代价是编程模型复杂化）。
Netty 补一句：EventLoop 组 + pipeline，铁律是慢操作必须丢出 EventLoop
...
把随机写变成顺序写，磁盘顺序吞吐接近内存量级；② 消费路径用 `sendfile`
...
2. **方法不是 public，或是 final/static**：CGLIB 靠子类覆写织入，
   覆写不了就织不进去。
3. **异常类型不对**：默认只对 `RuntimeException` 和 `Error` 回滚，
...
提交。两个坑都和「同一线程同时有两个事务」有关：① **自死锁**——外层
...
### D6：CAP 经常被讲错——给出你的版本，并解释为什么分布式锁需要 fencing token
...
而平时没分区的日子里，取舍其实是**延迟 vs 一致**（同步复制慢而一致，
异步复制快但有落后窗口）——这是 PACELC 对 CAP 的补全。
顺带纠偏一句：「MySQL 是 CA」这类说法没有意义，CAP 讨论的对象是
带复制的分布式系统，单机谈不上 P。

**一致性是光谱不是开关**：线性一致（整个系统表现得像单机）→
因果/会话一致（至少读到自己写的）→ 最终一致。实现上对应复制策略：
quorum（W + R > N）可到强读写，纯异步复制只能最终一致。
工程要点：**一致性是按数据分级购买的**——我项目里额度与交易走
单点强一致（主库 + 行锁），通知与读模型走最终一致（事件 + 重试收敛），
全系统买同一档既贵又慢。

**共识为什么难**：异步网络里无法区分「节点死了」和「网络慢」，
这是一切难题的根。朴素的 leader election 会脑裂：两个自认 leader 各写各的。
Raft 的直觉两句话：任期号单调递增 + **多数派**投票才能当选；
...
靠重试、outbox 重投和对账事后收敛——分区时选可用、容忍暂时不一致。
...
是「在我项目里怎么做」，这里是「脱离项目的机制本身」，两边互为弹药）。
...
2. **二叉/红黑树**：树高 log₂(N)，亿行 ≈ 27 层 = 27 次页读，磁盘上不可用——
   树高问题的本质是每个节点扇出太小，浪费了「一次 IO 拿回 16KB」的带宽。
3. **LSM 树**（RocksDB/Cassandra）：写优化（顺序写 memtable + compaction），
   代价是读放大和 compaction 抖动。InnoDB 选 B+ 树是读写平衡 + 事务集成
...
随机页上：页分裂频繁、buffer pool 局部性被打碎、写放大明显；自增或时间有序 ID
...
短期止血和长期根治分开。止血：`FORCE INDEX` / optimizer hint 钉住正确索引，
先恢复服务。根因通常两类：统计信息过期（大量写入后基数估算失真，
`ANALYZE TABLE` 重采样）或数据倾斜（某个值占了半张表，均匀性假设失效）。
...
1. 每行两个隐藏列：`DB_TRX_ID`（最后修改它的事务 ID）、`DB_ROLL_PTR`
   （指向 undo log 里的旧版本，串成**版本链**）。
2. 快照读（普通 `SELECT`）开始时生成 **Read View**：记录当时活跃（未提交）
   事务集合和高低水位。读每行时沿版本链回溯，找到第一个「对我可见」的版本
...
`SELECT ... FOR UPDATE`）读的是最新版本并加 next-key lock（行锁 + 间隙锁）
...
死锁与等待概率上升。防御：监控 `information_schema.innodb_trx` 里的事务时长
...
「一个事务内多次读必须自洽」——对账、批处理里先统计后处理的逻辑。
选型的正确表述不是「哪个级别好」，而是：**默认级别管普通路径，
关键路径用显式锁把语义钉死**——这样即使团队日后把默认从 RR 调到 RC，
钱路径的正确性也不随之漂移。这是我在项目里坚持显式 `FOR UPDATE` 的原因。
...
| 方案 | 一致性 | 代价 | 适用 |
| --- | --- | --- | --- |
| 2PC/XA | 强一致 | 同步阻塞、协调者故障时资源悬挂、吞吐差 | 极少：同厂商资源、低并发管理操作 |
| TCC | 业务层强一致 | 每个参与方写 Try/Confirm/Cancel 三接口，侵入大 | 资金类短事务，参与方可控 |
| Saga | 最终一致 | 补偿逻辑 + 隔离性丢失 | 跨服务长流程（开户、订单履约） |
| Outbox + 事件 | 最终一致 | 消费侧要幂等 | 「下游必须发生但不必立即」 |

**TCC 三大坑**（说得出这三个词才算真了解）：**空回滚**（Cancel 先于 Try 到达，
要能对「没 Try 过」的请求安全地 Cancel）、**悬挂**（Cancel 之后迟到的 Try
...
**对账兜底**，是资金系统的现实三层。真出现「跨资源必须强一致」的需求，
...
**止血与根治分开**：滚动重启是止血不是修复，重启前必须先留 dump，
...
选型轴是停顿目标：G1 是吞吐与停顿的平衡者（百毫秒级停顿、默认选择、
调优资料多）；ZGC 停顿亚毫秒且与堆大小无关，JDK 17+ 生产可用，
代价是吞吐略降、内存占用略高。授权链路 p99 敏感，ZGC 值得评估——
但**顺序不能反**：先用 GC 日志证明 p99 尖刺确实来自 GC 停顿，
而不是锁等待、连接池排队或下游慢（我项目的排查经验里，后三者更常见）。
拿数据说话：对比 GC 停顿分布和请求延迟尖刺的时间相关性，对上了再换收集器，
...
给堆外留的余量不足是被 OOMKilled 的第一大原因。

### E6：Java 内存模型（JMM）里 volatile 到底解决什么？虚拟线程又改变了什么？
...
- `ConcurrentHashMap`（JDK 8+）：CAS 插入空桶 + 桶头节点 synchronized，
  ...
  `get` 判空再 `put` 依然是 check-then-act 竞态，要用 `computeIfAbsent`。
  这和我项目里「check-then-insert 要靠唯一约束」是同一个竞态在不同层的投影。
...
系统的容量模型依然由最稀缺的下游资源决定。

### E7：API 怎么演进才不破坏调用方？分页为什么推荐 cursor 而不是 offset？

**答：**

**兼容性铁律**先说：加可选字段=安全；删字段、改类型、改语义=破坏。
...
重复或跳过记录，对账单、交易流水这种钱相关列表是正确性问题不是体验问题。
...
（curl 就能打）、网关/缓存生态、对外暴露自然。常见组合：对外 REST、
内部高频调用 gRPC。但要点破一件事：**换协议不豁免演进纪律**——
proto 的兼容规则同样严格（field number 永不复用、不改已有字段类型、
删除字段要 reserved），gRPC 只是把契约检查工具化了，没把兼容性问题
消灭掉。JD 提到 gRPC，能讲 proto 演进规则会是个小亮点。
...
快哈希 = 可爆破；慢哈希把单次验证抬到几十毫秒量级，爆破成本上升百万倍。
...
双密钥并存期（新旧同时有效）让轮换可以灰度做，不停机。
发卡语境下这直接对接 PCI DSS：持卡人数据环境（CDE）的密钥管理是审计的重头。

**JWT vs session**：本质是「状态放哪」的取舍——
...
金融场景的常见落地：**短命 access token（分钟级）+ refresh token 轮换 +
敏感操作 step-up 认证**（改限额、改地址时二次验证）——用短 TTL 补 JWT
吊销难的短板。JWT 高频错误清单：信任 `alg` 头（none 攻击）、
...
设计前提就是「泄漏一定会发生」，要设计的是爆炸半径：① 短 TTL 把
...
2. **非 INSTANT 的大变更用影子表工具**：gh-ost / pt-osc——建新结构的
   影子表 → 分批拷贝存量 + 应用增量（gh-ost 读 binlog，pt-osc 用触发器）→
   追平后原子 rename 切换。可限速、可暂停、可随时放弃。
   本质就是 S3 迁移在单表尺度的缩影：全量 + 增量 + 原子切换。
3. **应用层配合 expand-contract**：加新列（nullable/带默认）→ 代码双写 →
   回填 → 读切到新列 → 停写旧列 → 删旧列。每一步独立发布、独立可回滚。
   反例就是 B1 事故示例的根因类型：一次发布捆绑「改表 + 改代码」，
   回滚代码时表结构回不去。
...
文件层保证可追溯，执行层在评审时给大表 changeset 标注执行策略
...
能用的组件」：正确、线程安全、边界清晰。这一节每题给考点、答题骨架
...
1. `get` 忘了移动节点——那是 FIFO 不是 LRU，第一淘汰点。
...
把「记录访问」变成近似无锁。能说出「生产为什么不用裸 LRU」是这题的天花板。
...
顺势挂回：这就是项目里 Redis TTL + jitter 策略的单机镜像。
...
3. 两个参数的语义：capacity 表达允许的**突发**，refill 速率表达允许的
   **持续吞吐**——对齐项目里 burst 20 / sustained 10 rps 的配置语义。
...
按场景定——系统保护类限流 fail-open + 本地兜底，配额类 fail-closed。
...
无界 LinkedBlockingQueue 是隐患（任务堆积到 OOM），生产线程池要
...
结构化并发（`StructuredTaskScope`）让这段代码回归同步写法：fork 三个
...
本节是通用方法论层——答题时先用本节框架开场，再用项目场景落地举例，
两层配合就是「既有体系又有实战」的效果。

### P1：接口偶发超时（一天几十次、无规律），你怎么系统化排查？

**答：**

偶发问题的第一原则：**不要试图现场复现，先把「偶发」变成「可观测」**——
复现靠运气，观测靠工程。我的排查流水线：

1. **先把「偶发」定义清楚**：拉延迟分布直方图——双峰分布（大部分 20ms +
   一小撮 2s）和长尾渐变是完全不同的病。双峰几乎总是「撞上了某个离散
   事件」：GC、锁、缓存重建、超时重试。
2. **时间相关性分析**：把慢请求的时间戳导出来，和一切周期性事件对齐——
   GC 日志、定时任务（cron 表要真的翻一遍）、缓存 TTL 到期、连接池
   回收周期、部署时刻、整点流量。先问「尖刺有没有周期性」：
   有周期 = 有定时源头，一半的偶发问题在这一步破案。
3. **维度切分**：慢请求按实例/接口/账户/参数特征切——集中在单实例
   （宿主机或邻居问题）？单账户（热点行锁）？特定参数（某分支慢查询）？
   全随机（基础设施层）？切分结果决定下一步去哪里挖。
4. **请求级证据**：trace 采样必须**保留慢样本**（尾部采样，不是头部随机
   采样——1% 随机采样几乎永远抓不到 p99 样本）；日志 traceId 贯穿 +
   关键阶段耗时打点，让任何一条慢请求都能被解剖成「时间花在哪一段」。
...
异常的全事件流，就是为「事后取证」设计的；③ 嫌疑在网络层时 tcpdump
...
高频 watch 都有开销）；生产使用走审批留痕；`watch` 抓到的入参可能含
...
前 15 分钟的目标只有一个：**止血**，不是找根因。我的顺序：

1. **2 分钟定级**：影响面——错误率多少、影响哪类请求、**钱对不对**。
   资金正确性问题和可用性问题的处置优先级完全不同：可用性可以降级
...
3. **5 分钟止血决策**，按优先级：**回滚**（最近有变更就先回滚，
   不需要先证明因果——回滚便宜，猜错了再回来）→ **降级**（关非核心
   功能、开兜底）→ **限流**（保住部分容量）→ 重启（重启前留 dump，
   E5 的纪律）。止血动作要选**可逆的**。
...
已前进、依赖已切换——E9/S3 反复强调「每步可回滚」就是在防这个局面），
或根因百分之百确定且修复是一行级的；即便如此也要第二双眼睛看过再上。
补一句面试官爱听的：我评估一个团队的发布成熟度，第一个问题就是
「回滚要几分钟、多久真演一次」。

**追问 2：事后复盘（postmortem）怎么写才不流于形式？**
...
这种糊话；③ 根因避免**单根因谬误**——事故几乎都是多层防线同时失效，
要平行问三条线：为什么测试没拦住、为什么监控没先报、为什么止血花了
40 分钟；④ **action item 可验证**——「加强 review」是坏 action，
「schema 变更单增加回滚依赖检查项，owner 某某、deadline 某日」是好
action，且下次复盘先审计上次 action 的完成率。前提是 blameless：
...
4. **修「果」还要修「派生」**：错误数据可能已流进下游——缓存失效、
   ...
   画一张「污染传播图」逐个节点处理。
5. **闭环验证**：重跑圈定 SQL 应为零；再跑一轮业务不变量对账
   （S2 的第三层校验）确认没修出新问题。
...
**flaky test 零容忍**——出现即隔离（quarantine），限期修复或删除；
...
1. **发布前**：变更单写清三件事——改了什么、怎么验证成功、
   **怎么回滚**（含数据/配置的回滚依赖——E9 的 expand-contract 就是
   ...
   不是迷信，是出事时在场人力的期望值问题。
2. **发布中**：金丝雀/滚动——先 1 台或 5% 流量，盯 15 分钟核心指标
   （错误率、p99、业务量），**前进/回滚阈值事先写好**而不是现场凭感觉；
...
（降级开关、A/B，长期存在但有 owner 有台账）；② 每个 flag 的默认值
要经得起「配置中心挂了」的拷问——取不到配置时落到的那一侧必须是
...
## 6. 工程日常：选型、流程与效率（W1–W8）

区别于 P 节的「出事了怎么办」，这一节是「平常怎么干活」。面试官
（尤其 hiring manager）用这些问题判断你是不是**可持续交付**的工程师：
有自己的工作系统，还是全凭惯性。回答共同原则：具体做法 + 为什么 +
一个真实代价的反例——最忌答成教科书式的正确废话。
...
1. **澄清（半天内）**：用具体案例逼出精确需求（T5 的方法），
   产出一页行为描述让需求方确认——写下来的分歧最便宜。
2. **设计（深度按影响面定）**：小改动直接写；动数据模型/资金语义/
   跨模块的先写设计文档（W4），重点写失败场景和非目标。
3. **拆解与估算**：拆到 ≤2 天粒度（W5），不确定的部分先做时间盒
   spike；此时就识别哪些步骤有回滚依赖（E9 的 expand-contract 排期）。
...
6. **发布**：金丝雀 + 预设阈值（P6），发布后主动盯 30 分钟。
7. **收尾**：观测验证需求效果——不只是「没报错」，是「业务指标
   动了没有」；清理发布型 flag；顺手更新文档。
...
2. **总拥有成本**：学习曲线、运维负担（谁半夜起来修它？）、升级路径。
   注意运行时组件（数据库、消息队列）的成本远大于库/框架——
   引入一个新存储系统的真实含义是「从此多养一个孩子」。
3. **生态与生命周期**：社区活跃度、出现多久、大规模生产案例。
   我的默认立场是 boring technology：创新预算花在业务差异化上，
   基础设施用最无聊最成熟的——你的「创新代币」是有限的。
4. **退出成本**：单向门还是双向门？换一个 JSON 库是双向门，快速试错；
   选主数据库、定事件 schema 是单向门，值得花十倍时间——
   **决策的谨慎度应该和不可逆性成正比，而不是和讨论的热情成正比。**

流程上：真实候选 ≤3 个；时间盒 PoC 用**真实场景的数据和负载**测
（hello world 级 PoC 什么都证明不了）；结论写成 ADR——背景、选项、
决定、**被否决的选项和否决理由**。半年后新人问「为什么用 X 不用 Y」，
答案在仓库里，而不是在离职同事的脑子里。

挂回项目：资金互斥选 MySQL 行锁而不是 Redis 锁、事件发布选 outbox
...
（S3 的方法论按比例缩小）；④ 有没有渐进路径——能 strangler 就不推倒。
多数「重写冲动」在第①问就消解了：痛的根源常常是缺测试和文档，
换框架不治这个病。但也不当一刀切的守旧派——新技术命中真痛点时
（比如虚拟线程之于 IO 密集场景，E6），小范围试点拿数据说话。
...
不是运气**。真要换：沉没成本不参与决策，算的是「从今天起继续用 vs
迁移」的前瞻账；迁移执行走 S3 的双跑灰度。最重要的收尾是把教训写回
ADR：当初哪个假设错了——选型能力的成长几乎全部来自这类复盘。
...
2. **PR 控制在几百行内**：review 质量在几百行后断崖下跌——大 PR 得到的
   不是 review，是「LGTM 盖章」。小 PR 的复利链：review 快 → 合入快 →
   冲突少 → 回滚粒度细 → bisect 好用。
3. **原子 commit + 讲「为什么」的 message**：一个 commit 一个逻辑变更；
   message 写为什么而不是做了什么（做了什么 diff 已经说了）。这不是洁癖：
   `git blame` 是最常用的考古工具——半年后面对一行怪代码，blame 出来的
...
2. **非目标**：明确不做什么——防 scope creep 的关键段落（W1 追问）。
3. **方案**：数据模型 + 时序图 + 状态机，图优先于文字。
4. **失败场景推演**：每个环节「进程在这里挂了会怎样」——我项目所有
   机制文档都有这段，它逼出来的设计修正比评审会议还多。
5. **被否决的替代方案**：一句话方案 + 一句话否决理由——评审会上
   80% 的「为什么不用 X」被提前消灭。
6. **上线与回滚**：灰度方式、数据迁移、回滚路径（P6 变更单的前身）。

评审姿势：文档提前 24 小时发出（会上现读是集体浪费时间）；
...
1. 我们默认估的是**编码 happy path**的时间，而真实交付 = 编码 + 测试 +
   ...
   编码往往不到一半；
2. 计划谬误：人对「自己这一次」系统性乐观，对上一次的教训不敏感；
3. 不确定性不对称：顺利最多省 20%，踩一个坑能翻 3 倍——估算分布
   是长尾的，而我们直觉报出的是众数不是期望。
...
2. **用自己的历史数据校准**：我的个人系数（估 3 天实际 4.5 天 →
   ×1.5）比任何估算方法论都准，前提是记录过。
3. **对内计划紧、对外承诺松**：milestone 对外给日期区间不给单点。
4. **slip 纪律**：落后的第一时间暴露（完整故事在 B3）——
   估算错不是失职，隐瞒才是。
...
日常的**预防版**是两条：① 争取估算参与权——deadline 应该是
「业务需要 × 工程估算」的谈判结果；如果团队总是被单方面通知日期，
这个流程缺陷比任何一次 deadline 都更值得修。② 平时积累估算信用——
你的估算历史越准，谈判时你的数字越有分量；信用是复利资产，
...
一个公式：**效率 = 反馈回路速度 × 专注保护**。所有做法都在优化这两项。
...
新人的首要任务不是证明聪明，是打通自己的交付管道。

### W7：AI 辅助开发，它真正提效在哪？带来了什么新问题？

（T6 讲质量与责任红线；这题是效率与能力维度，两个角度都可能被问。）
...
1. **验证成本转移**：生成变快了，但幻觉 API、过时用法、似是而非的
   并发代码把工作量转移到验证端——净收益取决于你的验证效率，
   这解释了为什么资深者从 AI 获益更多。
2. **代码膨胀**：AI 偏爱防御性样板和过度抽象，不约束的话代码量和
   review 负担同步上涨——对策是 prompt 强制「最小 diff」+
   把项目约定喂进上下文。
3. **审查疲劳**：大量「看起来都对」的代码降低警惕性——这恰恰是
   微妙 bug 混入的完美环境，所以钱路径的 AI 产出我逐行重写而不是浏览。
4. **技能维护成本**：调试直觉和手写能力长在「挣扎」里，全托管会退化——
   我刻意保持核心逻辑手写，定期不带 AI 练手感（C 节就是这么准备的）。
5. **团队方差放大**：会验证的人效率翻倍，不会验证的人更快地产出
...
1. **有意的战术债**（「先硬编码，下月抽象」）：合法工具，但必须
   **打欠条**——TODO 关联 ticket 或进债务台账；不记录的债不是债，
   是未来的惊喜。
2. **无意的债**（当时不知道有更好做法）：靠 review 和后见发现，
   发现即记录。
3. **腐化债**（当年正确，世界变了）：依赖过期、模式失配——要定期
   巡检，「升级依赖」是最容易被无限推迟的一类。

管理机制四条：

1. **台账带「利息」标注**：这笔债每月吃掉多少（摩擦工时、bug 占比）。
   没有利息的「债」可能根本不用还——丑但稳定且不再改的代码，
   还它是伪工作，先还利息最高的。
2. **还债两条腿**：小债顺路修（boy scout rule——但顺路修必须小，
...
3. **翻译成业务语言**（T2 同款）：谈判用摩擦数据不用道德词汇——
   ...
   而它承载主业务」可以。
4. **控制新增比还旧账便宜一个量级**：definition of done 包含测试
...
或量化摩擦做杠杆——P3 复盘的 action item 是技术债最好的立项通道
（「这个季度 40% 的线上问题出自这个模块」没人能反驳）；
③ 守住职权底线：**新代码不欠新债不需要任何人审批**，这是工程师
自己说了算的范围。三条做完债还在雪崩，那是组织级问题——
值得在逆质问（R5）里提前探明。
...
需求澄清（5 min）→ 规模估算（5 min）→ 核心设计（20 min）→ 失败模式与追问（10 min）。
...
2. 预算语义：10 亿 pt 是**硬上限**（超发=事故）还是**软目标**（超发几万 pt 可接受）？
   ——这是整题的分水岭，先按「近似硬上限：不允许显著超发，瞬时少量超发可事后核销」谈。
3. 规则：按金额比例（如 5%）、有单笔上限、每人活动期间总上限。
4. 规模：大促峰值支付 10,000 TPS（锚点假设），活动 24 小时。

**阶段 2：规模估算**

- 峰值 10,000 TPS 支付事件 → 积分计算 10,000 TPS，写入积分账本同量级。
- 10 亿 pt ÷（平均每笔 50 pt）≈ 2,000 万笔就烧完预算——**大促开始后几小时内**，
  所以「预算还剩多少」是一个每秒被扣减上万次的热点计数器，这是本题的心脏。
...
1. **幂等入口**：inbox 表按 `event_id` 唯一约束去重——支付系统重发、Kafka 重复投递
   都不会重复发积分。和我项目 Inbox 完全同构。
2. **规则计算**：纯函数算出应发 pt（比例、单笔 cap、查该用户已发总量做个人 cap）。
3. **预算扣减（热点核心，展开讲）**：
   - 反面方案 A：DB 单行 `UPDATE budget SET remaining = remaining - ?
     WHERE remaining >= ?`——正确但单行锁串行化，几千 TPS 就到顶。
   - 反面方案 B：Redis `DECRBY`——扛得住量，但 Redis 是缓存层，
     宕机恢复后计数从哪来？钱相关的 source of truth 放 Redis 是我一定会被追问的红线。
   - **推荐：预算分片 + 分层扣减**。把 10 亿 pt 切成 N=1000 个 shard（每片 100 万 pt）
...
2. 消费积压：积分是异步的，积压不影响支付主链路——这是把发积分放 Kafka 后面的
   最大收益；监控 consumer lag，App 端积分展示「计算中」兜底。
...
这里正好展示「按竞争域选并发策略」：全局预算靠分片摊薄，个人上限靠分区串行化，
不是所有热点都用同一把锤子。
...
对账作业：活动结束后（以及活动中每小时增量地）扫 `payment.completed` 全量事件
...
3. 汇总层：文件尾部的合计金额/笔数 ↔ 内部当日 posting 合计 ↔ 实际结算资金
   （银行侧入出金）。明细全对但汇总不对 = 解析漏了记录，汇总对但明细有差 = 有互相抵消的错误，
   **两层都要**。

**阶段 3：处理管道**

1. **文件接收与不可变落地**：原始文件先原样存对象存储（审计要求：任何结论都能
...
2. **对账系统自身的正确性**：对账是「裁判」，裁判错了比球员错了更糟。
   所以匹配引擎必须是**纯函数式、可重放**的：同样的输入文件 + 内部快照，
   任何时候重跑得到同样的差异清单；修 bug 后对历史重跑验证。
3. **结果的消费方**：差异不是报表就完了——「清分有授权无」要真实补扣额度，
   对账系统需要有回写主系统的通道，这个通道本身要幂等（差异 ID 作幂等键）。
...
1. **先定性再动手**：这 1,000 笔是「解析漏了」（对账系统的错）还是「posting 链路
   ...
   十分钟内分清是哪边——方向错了后面全白干。
2. 若是主系统丢数据：**先止血**——确认丢失是否仍在发生（看 posting 消费链路的
   lag/错误率），在扩大前停住；然后评估用户影响——少记入账意味着用户额度
   比真实多、账单会少——**宁可晚修不可错修**，补记走正常幂等入口重放这 1,000 笔，
   绝不手写 UPDATE。
3. 修完后：对补记结果再跑一轮对账闭环验证；事后复盘为什么监控没有在对账之前
...
而且是区分「做过题」和「懂工程」的题——答案的重心在**风险编排**，不在新系统多漂亮。

**答：**

**总原则先亮出来**：发卡系统迁移的第一目标不是「新系统上线」，
是**任何一天出问题都能在分钟级回到旧系统**。方案围绕三个词：strangler（绞杀者模式）、
dual-run（双跑比对）、reconciliation（每步都有对账闸门）。
...
1. 旧核心的 DB 通过 **CDC**（binlog/日志捕获）实时复制到新平台的数据模型，
   写一个转换层做新旧模型映射（旧系统的字段语义考古是迁移里最耗时的部分，
   要配一个「旧系统语义字典」文档工程）。
2. **全量回填 + 增量追平**：先跑历史全量（按账户分片、可断点续跑——
   claimable job 模式又一次适用），再让 CDC 增量追平，差值收敛到秒级。
3. **每日对账闸门**：新旧两边按账户跑余额/状态核对，差异率不降到阈值以下，
   禁止进入下一阶段。对账脚本在迁移第一天就写，不是切流前一周补。

**阶段 3：读路径切流（strangler 开始咬）**

入口加路由层（API gateway / 前置 facade），账单查询等读流量按
**账户维度灰度**（1% → 10% → 50% → 100%）切到新平台；新平台读自己的复制数据。
出错的爆炸半径 = 展示错误，可即时回切。这一阶段真正的产出是：
...
1. **双写的一致性陷阱**：双写期间两边都可能各自失败——必须明确
   ...
   只报警、由对账修复，**绝不**搞跨系统分布式事务去追求双写强一致。
2. **长尾业务**：分期中的账户、争议中的交易、督促中的债权——这些跨月的
   长事务状态最难迁，宁可设计「存量在旧系统跑完、增量进新系统」的双轨期，
   也不硬迁到一半状态。
...
就意味着 STIP 代授权期，损失敞口打开。灰度 + 双跑把同样的风险摊到几个月里，
...
而是接受行级最终一致 + 用**账户级校验**兜底——校验的单位和业务不变量的
...
**流量复制的三种方式**（按真实度与安全性权衡）：

| 方式 | 真实度 | 延迟 | 风险 |
| --- | --- | --- | --- |
| 网关镜像（请求复制一份异步发新系统） | 高（原始请求） | 实时 | 要防新系统产生副作用 |
...
前进/暂停/回退条件**事先写进 runbook**（如「新侧错误率高于旧侧 0.1pp
即暂停」），推进决策照单执行，不靠会议室现场拍板。
...
| --- | --- | --- |
| 成功 | 成功 | 正常 |
| 成功 | 失败 | 用户无感；差异进补偿队列异步重写旧侧（幂等）；旧侧落后期间**回切能力降级**——这就是 S3-c 说的健康度指标 |
...
已完成阶段的收益不会蒸发。调整动作：① 重排剩余能力域，只继续迁
「留在旧系统成本最高」的域（维护人力、故障率、合规风险最大的），
其余域**停在双轨稳定态**；② 把「停留成本」量化给决策层——双系统并存的
运维费、双实现税、专家占用，让「不继续迁」也是一个被定价的选择而不是
默认免费；③ 守住一条设计底线：迁移的每个中间态都必须是**可长期运行的
...
下面每题给出**回答骨架 + 一个可参考的完整示例**。示例是通用后端场景的样板，
...
（按分钟叙述，突出「先止血后修复」）→ 根因 → 你主导的事后改进 → 一句反思。
全程用「我做了什么」，不用「我们」模糊自己的贡献；影响必须有数字。
...
> 我的处置：第一步不是找根因，是**止血**——先回滚刚发的版本，5 分钟内完成，
...
「索引变更是我评审通过的，我当时只看了它对查询的收益，没有问『回滚代码时
这个索引怎么办』——这是我的评审盲区，不是流程的错。流程改进只是让下一个人
...
> 工作没有上线。复盘下来核心失败原因在我：我在没有充分验证「用户是否真的
...
> 授权和清分为什么分离、账单为什么要快照，把行业概念翻译成状态机和时序图；
> ② 再啃工程——幂等、事务边界、行锁、outbox/inbox，每个机制我都写了
> 「如果去掉会发生什么」的反向推演，确保学的是因果而不是模式名；
> ③ 最后自我审查——模拟「进程在任意一行宕机」做穿越测试，找出并修掉了
> 若干活性缺陷（比如错过定时触发时刻后的补偿）。两个月内从对发卡一无所知
> 到能对链路上每个设计决策说出为什么、以及不这么做会怎样。
> 我的学习模式总结起来是：用「构建一个能跑的东西」倒逼理解，
> 用「反向推演」检验理解不是错觉。

**追问：进了新团队，面对没有文档的存量系统，这套方法怎么用？**

**追答骨架：** 「核心不变：构建 + 反推，只是对象从新项目换成存量系统。
落地动作：① 先追一条最重要的调用链从入口读到落库，边读边画时序图——
...
或按团队的 tie-break 规则（比如 code owner 决定）。我不会为了和谐直接照改
...
事中只做止血决策，根因留给白天的脑子。我在自己项目里给每类 worker 写
排障笔记，就是在练这个习惯。（前 15 分钟怎么走的完整版见 P3。）

**日常**：三件事让 on-call 可持续——
① 每次报警都要有下文：要么修根因，要么调阈值，「报了又不用管的警报」
是压力的最大来源，我会主动清理 alert 噪音；
② 演习：故障注入过的场景，真发生时就不是压力而是流程；
③ 诚实的复盘文化：无责复盘（blameless postmortem）让事故变成组织资产，
如果事故会导致追责，人就会隐瞒小问题直到它变成大问题。
个人层面，值班周我会主动降低其他承诺密度，保证睡眠冗余——
压力管理的大头其实是精力管理。
...
1. **用具体案例逼出精确**：拿到模糊需求，我不问「能不能说清楚点」，
   而是构造具体场景反问——「用户还款 5,000 円但账单是 4,800 円，多的 200 怎么处理？」
...
3. **对变更分级响应**：影响数据模型/资金语义的变更要正式评审（这类改错代价大），
   展示层变更快速接受。频繁变更多数时候说明业务本身在探索期，
   那么架构上我会把「稳定的核心」（账务、状态机）和「易变的边缘」（规则、展示）
   分层——核心慢改、边缘快改，变更就不再是痛苦而是常态。
...
1. **用在哪**：样板代码、测试用例的边界枚举（AI 特别擅长帮我穷举
   case）、陌生 API 的用法探索、文档初稿、以及「换个视角 review 这段代码」。
   我准备面试的这个项目就是这个工作流：设计与取舍全部我拍板，
   AI 加速验证和初稿，我逐行改写定稿。
2. **责任模型不变**：提交者对代码负全责——「这是 AI 写的」永远不是
   缺陷的借口。AI 产出我按「陌生人的 PR」标准 review；资金语义相关的
   代码（计息、幂等、事务边界）我必须能逐行解释为什么这么写。
3. **硬红线**：生产数据、密钥、客户信息不进 prompt——等价于发送给
   外部服务，PCI/个人信息保护的合规逻辑直接适用；AI 生成的 SQL/DDL
   进生产前和人写的走同一套评审闸门（E9 的执行策略标注一视同仁）。
4. **质量兜底靠测试不靠信任**：行为用测试钉死（幂等、并发、边界），
   而测试的断言我要亲手确认有意义——AI 会写出「跑得过但什么都没验证」
   的测试，这是我见过它最隐蔽的坑。

一句话立场：AI 改变我的产出速度，不改变我对正确性的所有权。
金融后端恰恰是「验证成本高于生成成本」的领域——懂领域、会验证的
工程师，价值反而被 AI 放大。（效率与能力维度的另一半——它真正提效在哪、
...
1. **业务位置独特**：日本正处在政府推动的 cashless 转型中段（支付比率从
   两成级别爬到四成、目标八成，锚点数字面试前更新），而 PayPay 集团站在
...
2. **技术问题是我想解的类型**：发卡系统是「正确性和规模同时苛刻」的领域——
   ...
   工程挑战（这是最能沉淀架构能力的场景）。我为准备这个方向做了一个
...
1. **集团结构**：PayPay 码支付（注册用户 6,000 万量级）与 PayPay カード
   （有效会员 1,000 万+ 量级）咬合——卡是生态的授信底座，
   「PayPayクレジット（旧あと払い）」把授信直接嵌进扫码体验，
   没有实体卡也在使用卡公司的授信。对后端的含义：同一套额度和账务
   要同时服务卡组织报文和自家 App 流量，两者的幂等键（网络交易标识 vs
   客户端 idempotency key）、限流策略、峰值形状（营销日历驱动的尖峰）
   完全不同。
...
我不知道的约束——这正是我逆质问想请教的部分」（顺势衔接 R 节，
把「被考」转成「对话」）。
...
红线：不要把回答建立在对母国的否定上（面试官听起来像推力驱动，推力会变），
...
我把多语言环境当工程问题处理——降低对语言能力的依赖，提高对**沟通结构**的依赖：
...
把它理解成低效的官僚流程。我的理解：**根回し的本质是把冲突前置到成本最低的
...
会议变成确认而不是遭遇战。同时我会守住一条工程底线：合意形成不能变成
...
- **R1**：「授权链路的 p99 延迟预算在团队内部是怎么分配的？外部依赖（风控、
  信用情報機関）占掉多少，超预算时的降级决策由谁做？」
  ——暴露你懂延迟预算是分配问题、降级是权责问题。
- **R2**：「遗留系统现代化目前走到哪个阶段？新旧系统并存期的对账和回切
  机制是怎么设计的？」——直击他们最痛的日常，也验证 S3 准备的方向。
- **R3**：「团队的事故复盘文化是什么样的？最近一次让大家学到最多的事故
...
- [ ] 锚点数字用最新公开资料刷新（PayPay 用户数/卡会员数/cashless 比率/
      不正利用被害额/interchange 公开料率——用在 G1 及其追问和 D1）。
- [ ] E 节：每题练出「60 秒结论先行」的短版——通用技术题先给结论，
      面试官想深挖自然会追问；确认每题至少能讲一个「不这样会怎样」的反例。
- [ ] C 节：五道手写题**真的动手写一遍**（看会 ≠ 写得出）；LRU 和
      阻塞队列要能 15 分钟内白板写对，`while` 条件等待这类淘汰点零失误。
- [ ] P 节：每题的「顺序」就是分数——止血先于根因、堵源头先于修存量、
      回滚先于 fix forward；给 qa bank 第 12 节的每个项目排障场景
      配一个本节框架的开场白，形成「方法论 + 实例」的组合拳。
- [ ] W 节：W1 的七步流程当骨架背熟（它是 hiring manager 综合题的目录页）；
      W2 的四维选型、W5 的低估三成因、W7 的提效五项/问题五项能脱稿列举；
