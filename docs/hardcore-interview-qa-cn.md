...
回答共同纪律：**60 秒结论先行lead with the conclusion**，面试官想深挖自然会追问；每层深入都带一个
...
引用文档拒绝是执行纪律execution discipline。我准备面试的这个项目，每份机制文档都有明确的
scope 边界，就是这个习惯的练习场。
...
**阶段 1：需求澄清requirements clarification（先问再答，面试官等你问）**
...
**阶段 4：失败模式自查failure-mode review（不等面试官问，主动讲）**
...
面试语言：我优化的不是期望工期，是尾部风险tail risk。
...
（面试官这一问是在测：你是否把事故归因于个人英雄主义attribute the incident to individual heroics，还是推动系统性改进systemic improvement。）
