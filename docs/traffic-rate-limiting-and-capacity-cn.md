...
Kafka 的 6 不是一个全局 concurrency 值，而是当前 listener containers 的上限合计：

```text
Notification: 3 个 listener × concurrency 2 = 6
合计最多 6 个 consumer threads
```
...
> [!WARNING]
> 80 个 Tomcat 线程、6 个 consumer 线程和 20 个 worker 并不等于系统能同时做 106 次 DB 工作。
