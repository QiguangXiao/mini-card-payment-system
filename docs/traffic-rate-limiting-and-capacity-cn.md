...
Kafka listener threads                 当前最多 11
Outbox workers                         4
...
Kafka 的 11 不是一个全局 concurrency 值，而是当前 listener containers 的上限合计：

```text
Notification: 4 个 listener × concurrency 2 = 8
Risk feature: 1 × 3 = 3
合计最多 11 个 consumer threads
```
...
> [!WARNING]
> 80 个 Tomcat 线程、11 个 consumer 线程和 20 个 worker 并不等于系统能同时做 111 次 DB 工作。
