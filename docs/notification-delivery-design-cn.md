...
> Resilience4j, CircuitBreaker, Retry, intra-attempt vs durable retry, delivery sender,
> 配信(はいしん), 冪等性(べきとうせい)。
...
| `NotificationDeliverySender`(push/email) | 单渠道地址/文案组装 + provider 调用（当前模拟） | — |
| `ResilientCallHelper` | 只封装 Retry + CircuitBreaker 的重复装饰代码 | — |
| `SimulatedProvider` | provider-side idempotency / latency / failure demo | — |
...
业务事务(posting/statement/...) ─Outbox→Kafka─▶ XxxNotificationListener
  ...
  NotificationDeliveryWorker(worker 线程, 事务外):
     providerMessageId = senderByChannel[channel].send(delivery)
       # sender 内部：合成/查询地址 + 组装文案 + Retry(CircuitBreaker(provider.send))
     [短事务] lockCurrentLease → markSent(providerMessageId) / markFailed → update
...
`NotificationDeliverySender.send(delivery)` 把它一路传到模拟 provider；`SimulatedProvider` 用一个
...
```
Retry( CircuitBreaker( provider.send ) )
```
...
- **Timeout 不放在 helper 里**：真实 HTTP provider 的 connect/read timeout 应由 HTTP client/SDK 配置。
  这里不使用 TimeLimiter，因为它需要额外线程池和 async future；为了一个 demo provider 把同步 worker 链路重新复杂化，
...
`creditAccountId`）——这是“还没有 User/Customer 聚合”的已知缺口。当前代码选择**不再提前抽
...
`SimulatedProvider` 替代真实 SDK，两个 sender 只负责合成地址/文案并调用它。这个小组件刻意做三件用于演示/教学的事：
...
3. **可注入延迟**（`simulated-latency-millis`）→ 让 slow-call CircuitBreaker 与 lease/retry 的关系可被观察到。

接真实 provider 时替换 sender 内部的 provider 调用（可用项目已有的 OpenFeign，或 WebClient），worker/状态机不动。默认 `failure=0`、
...
worker(D_push): sender 合成 token+文案；Retry(CB(provider.send)) → push-provider ack(R1)
...
通知发送不再有额外 sender pool；真实 provider 超时交给 HTTP client/SDK，避免为了 TimeLimiter 把同步链路拆成
async future。
...
- **provider 仍是模拟**：真实化只换 sender 内部 provider 调用 + Resilience4j 阈值调参；OpenFeign 已在依赖中。
