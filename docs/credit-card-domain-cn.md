...
- 不是外部实时请求，而是周期性批处理。当前产品级固定日期：**每月月末关账，次日由 `BillingCycleScheduler`（每天 JST cron）判断是否关账日并触发出账，次月 27 日为扣款基准日；27 日非日本营业日则按 `JapaneseBusinessDayCalendar` 顺延到下一营业日**。
- 当前实现（claimable job，详见 `claimable-jobs-cn.md`）：
  - `BillingCycleScheduler` 每天 cron 触发 → `StatementCycleService` 计算 billing cycle（period/dueDate）、按账户数算 shardCount、`INSERT IGNORE` 建分片 `statement_jobs`。
  ...
  - `statements` 存账单汇总，`statement_items` 存交易快照；`card_transactions.statement_id` 标记交易已进入哪期账单（防重复出账）。
  - 同事务写 `AUTO_REPAYMENT` DelayJob 计划 dueDate 自动扣款；`statement.closed` 经 Outbox 发布，`StatementNotificationListener` 消费创建 `STATEMENT_CLOSED` 通知（未来 PDF/还款提醒也可消费）。
...
- 当前实现（简化版 Repayment）：
  - `AUTO_REPAYMENT` DelayJob 到期 → `AutoRepaymentDelayJobHandler` → `AutoRepaymentService`；当前不建 `bank_accounts` 表，假设已有默认扣款授权；`SimulatedBankDebitGateway` 默认 `SUCCESS`（配 `FAILED` 时不入账，交 DelayJob retry/DEAD）。
  - 成功后用确定性幂等键 `auto-debit:{statementId}` 调 `RepaymentService.receive(...)`；`POST /api/repayments` 用 `Idempotency-Key` 防重复还款。
  - `RepaymentService.receive(...)` 在**同一 transaction boundary** 内更新 `repayments`、`credit_accounts.posted_balance`、`statements.paid_amount/status`；**锁顺序 `credit account → statement`**，避免和账单生成相反锁顺序。
  - `repayment.received` 经 Outbox 发布，Notification 创建 `REPAYMENT_RECEIVED` 通知，Ledger 记 `REPAYMENT_RECEIVED/CREDIT` 分录。
...
当前项目已实现**最小 Ledger projection**（学习用，非生产总账）：
...
每条 entry 关联 `source_event_id + entry_type` 并用 `consumer_inbox` 做第一道幂等；不做完整会计科目、不做复式借贷平衡、不做结算。authorization 事件**不**记 ledger（授权是 credit hold，不是 posted receivable）。生产级 Ledger 还需 accounting account、journal balance、fee/interest/refund adjustment 和更严格审计。
...
→ Minimal Ledger → Authorization Expiry → Outbox/Kafka notification & risk projection
```
...
> 这不是普通 TODO 清单，而是服务金融后端 interview 的 **remaining domain roadmap**。当前已覆盖 interview 最核心的主链路（authorization → posting → statement → repayment → notification/risk/ledger → outbox/inbox/delayjob），所以**不建议继续无差别堆功能**——只补最能帮助理解金融后端概念的小切片，其余先停在文档解释层。

| 优先级 | 领域 | 推荐范围 | 为什么值得做 / 为什么不是更前 |
| --- | --- | --- | --- |
| **Done** | 最小 Ledger | 已实现为学习 projection（见 §4） | 帮你区分交易流水/账单/还款/余额/会计账本；可复用 Outbox/Inbox 思路学 downstream projection |
| **P1** | 最小 Reconciliation | 本地 CSV/内存模拟 network/bank statement，对比 `card_transactions` 或 `repayments`，输出 mismatch report；**不自动改账、不接真实文件** | 对 PayPay Card 这类金融后台很贴近真实工作；能解释为什么金融系统需要 operational exception handling。**不更前的原因**：没有 ledger 或稳定 internal records 时，对账会退化成普通列表 diff；interview 价值在"为什么对不上、如何处理 exception"，不是 CSV 解析 |
...
**建议路线**：先消化现有主链路 → 修文档过期点 → 最小 Ledger（已完成）→ 最小 Reconciliation → 停下来复盘 interview 表达。如果只再补一个领域，选**最小 Reconciliation**；补两个则 **Reconciliation + Authorization Reversal**；时间有限则 Ledger 已足够作概念入口、Reconciliation 先只讲概念。
