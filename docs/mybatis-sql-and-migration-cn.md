...
### 6.3 当前文件结构（baseline reset 后）

```text
src/main/resources/db/changelog/db.changelog-master.yaml      ← 入口，只 include 当前 baseline + seed
...
- `0001` 已经是当前最终 schema：`statement_jobs` 是 flattened 形态，notification 已拆成 intent + delivery，Outbox/DelayJob/NotificationDelivery 都有 `lease_token`，statement 有 `version`。
- 旧的 `0002` 到 `0010` migration 已从 active changelog 移除，避免新库初始化时先建旧结构再反复 ALTER/DROP。
