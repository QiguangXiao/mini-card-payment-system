# 高流量系统设计手册（High Traffic System Design）

> **归档对齐说明（2026-07）**：本文已按当前代码、配置和异步拓扑逐段校准。历史 `card_risk_features`、Risk/Ledger projection consumer、statement/repayment Kafka 通知均已删除；当前 Kafka 只承载 authorization/transaction 通知事件，listener 落 Inbox + notification intent/delivery，provider 由 durable worker 投递。容量数字会明确区分“当前 guardrail”和“假设/压测起点”。现行容量与限流设计以 [traffic-rate-limiting-and-capacity-cn.md](../traffic-rate-limiting-and-capacity-cn.md) 为准。
...
> 高流量下我会缩短 account `row lock` 的 critical section：external risk 在账户锁前，Kafka publish 经 Outbox 在 commit 后执行，通知先落 durable intent/delivery 再调 provider；读扩展只缓存可重建 snapshot；后台 worker 使用 bounded pool、lease 和 retry。观测上看 p95/p99、DB lock wait、Hikari pending、Kafka lag、Outbox/DelayJob/StatementJob/NotificationDelivery backlog、Redis error 和 JVM GC。
...
| API token bucket | `AuthorizationRateLimitInterceptor`, `RedisTokenBucketRateLimiter`：每 IP capacity 20、refill 10/s、fail-open | body 解析和事务前返回 429；这是系统保护，不替代业务幂等/行锁 |
...
| 后台 worker 有界 | `outbox.publisher.worker-pool-size=4`, queue `100` | 下游慢时不无限堆线程和内存 |
| Worker ownership | `FOR UPDATE SKIP LOCKED`, `PROCESSING lease`, `lease_token` | 正常 lease 内避免多实例并发 claim；超时可重领，迟到旧 worker 不能覆盖新 owner |
...
| Notification delivery | Inbox 后同事务写 intent + per-channel delivery；worker 4、batch 40、每渠道 20/s | Kafka listener 不被 provider I/O 占住；durable retry 与 provider 短 retry 分层 |
| Runtime guardrail | Tomcat max 80、Hikari max 10、external risk bulkhead 4 | 入口并发不等于 DB/provider 吞吐；最窄共享资源决定排队 |
| Kafka partition | authorization/transaction topics 各 3 partitions；两个 listener container 各 concurrency 2 | 当前单进程最多 4 个 consumer threads；partition key 分别是 authorizationId/cardTransactionId |
...
| 认证后 client/account/merchant 级限流 | 当前只有 remoteAddr token bucket + card velocity；生产还需要可信身份和业务热点维度 |
...
| Kafka consumer | consumer lag, DLT, slow Inbox/intent write | 两个 Notification listeners；provider backlog 要另看 `notification_deliveries` |
...
边界要说完整：`authorize` 是单个 `@Transactional` 方法，external risk 发生在 idempotency winner 的 authorization row 已 `FOR UPDATE` 之后。因此它虽然不占 account lock，仍占 Tomcat thread、Hikari connection、事务和 authorization row lock；同一 idempotency key 的 loser 会继续等待 winner。当前 semaphore bulkhead 4 控制的正是这段 brownout 爆炸半径。
...
- Notification listener 用 Inbox 幂等，并在同一事务创建 intent + per-channel delivery rows。
- NotificationDelivery worker 在事务外调 provider，再用短事务 finalize `SENT/retry/DEAD`。
...
```text
同一 notification intent 可能被重复创建
provider 可能收到同一 delivery 的重复请求
```

当前第一层由 `consumer_inbox` 和 `notifications.source_event_id` unique 去重；第二层是 at-least-once delivery：worker 在 provider 成功、DB finalize 前宕机时仍可能重发，所以 delivery id 要作为 provider idempotency key。Inbox 能阻止重复创建 work row，却不能神奇消除已经越过本地事务边界的外部副作用。
...
Thread 1 enters StatementReadService.get(stmt-001)
  ...
  -> reads Redis miss
  -> SET rebuild-lock NX PX 2s wins
  -> loader calls DB read service
  -> writes L1 and Redis
  -> compare-and-delete releases only its own token

same pod Thread 2..100 wait on Caffeine per-key loader
other pods that lose Redis lock poll L2 5 times x 20ms
  -> reuse winner result when visible
  -> wait exhausted / Redis error then fallback DB for availability
```
...
- Redis `SET NX PX` rebuild lock 的跨 Pod single-flight，TTL 2s、等待 `5 × 20ms`
- L1 Caffeine + Redis L2
- L1 30s、L2 5m + 30s jitter、version CAS + 10s tombstone

局限：

> rebuild lock 是削峰而非 correctness lock。锁过期、Redis 故障或 loser 等待耗尽时会回源 DB，因此不能承诺全局严格一次加载；正确性仍靠 DB source of truth、版本 CAS 和 tombstone。跨 Pod L1 Pub/Sub 当前默认关闭，其他 Pod 的旧 L1 最长由 30s local TTL 兜底。
...
Hikari max pool = 10
external risk semaphore bulkhead = 4
```

如果每个 authorization 都需要 DB connection，而请求同时到达 80 个：

- 最多 10 个拿到 DB connection，其余可能在连接池或更前面的资源边界等待/失败。
- external risk brownout 最多允许 4 个同线程调用进入，但这 4 个仍可能持有事务和 Hikari connection。
- Tomcat thread 被占住。
- p99 上升。
...
- authorization 与 transaction topics 各 3 partitions。
- `AuthorizationNotificationListener` container concurrency 2。
- `CardTransactionNotificationListener` container concurrency 2。
- 两者同属 `mini-card-notification-v1` group、订阅不同 topic，单进程最多 4 个 consumer threads；没有 Risk/Ledger consumer。
...
- pool 固定，避免无限线程。
- queue 有界，避免无限堆内存。
- queue 满时，poller/dispatcher 捕获 `TaskRejectedException` 并恢复 durable row 的可见性。Outbox/DelayJob/Statement 走失败次数与 retry/DEAD；NotificationDelivery 因 provider 尚未调用，只释放 lease 并延后，不消耗 delivery attempts。
- Outbox、普通 DelayJob、AutoRepayment、StatementJob、NotificationDelivery 五个 executor 各 4 线程/queue 100，慢机制不会直接占满另一机制的 worker；但需要 DB 的部分仍共享 Hikari 10。
...
| Consumer | Lag 影响 |
| --- | --- |
| Authorization notification | 授权通知 intent 延迟创建，主交易正确性不受影响 |
| Transaction notification | 入账通知 intent 延迟创建，主交易正确性不受影响 |

Kafka lag 低也不代表用户已收到通知：listener 只创建 durable work，provider 侧积压要看 `notification_deliveries` 的 oldest `PENDING`、`PROCESSING` lease age、`DEAD` 和发送 latency。DLT 当前没有自动 monitor/replay consumer，进入 DLT 后必须由告警和 runbook 接管。
...
当前 external risk 决策是同步依赖，Redis velocity 是 60s/max 3 的辅助信号且故障时 fail-open；二者都不是已删除的 Kafka risk projection。账务 source of truth 直接落 MySQL 业务表，不存在一个可任意延迟的 Ledger consumer。
...
DelayJobPoller -> bounded delayJobWorkerExecutor / autoRepaymentDelayJobWorkerExecutor
StatementJobDispatcher -> bounded statementJobWorkerExecutor
NotificationDeliveryPoller -> bounded notificationDeliveryWorkerExecutor
```
...
- 不无限创建线程。
- 不无限堆内存。
- 让 durable row 回到可调度状态；不同机制按自身语义决定是否消耗一次 attempts，不能用一个统一“失败计数”概括。
...
当前 statement GET cache 同时做 JVM 内和跨 Pod single-flight：

```text
Caffeine Cache.get(statementId, loader)
-> Redis SET rebuild-lock token NX PX 2s
-> loser 最多等待 5 次，每次 20ms，观察 L2 是否由 winner 填充
```

作用：

- 同一 pod 内，同 key L1/L2 miss 时由 Caffeine 合并加载。
- 跨 Pod 正常只有 Redis lock winner 回源 DB，loser 等 L2。

局限：

- 锁 TTL 过短可能在 winner 未完成前放出第二个重建者；当前 2s 必须大于正常 DB load + L2 write。
- loser 只等约 100ms；等待耗尽或 Redis 异常会回源 DB，以 availability 换有限的重复读取。
- 解锁用 token compare-and-delete，防止旧 owner 删除新 owner 的锁。

生产拓展：

- 按实际 DB load p99 调整 rebuild lock TTL/wait budget。
...
> [!WARNING]
> Redis rebuild lock 只能保护 cache rebuild，不能保护 authorization 额度正确性。

### 9.3 Cache avalanche

当前 statement read cache 对 Redis TTL 加 jitter：
...
- 主 API 只写 Outbox。
- 当前两个 Notification listeners 是异步 side effect 的 durable 创建入口。

但长期有业务风险：

- Notification 晚到。
- `notification_deliveries` 继续积压，即使 Kafka lag 已恢复也可能尚未送达 provider。
- Outbox 表膨胀。
- DLT 未处理会隐藏数据问题。
...
| Topic | Key 口径 | 原因 |
| --- | --- | --- |
| authorization events | `authorizationId` | 同一 authorization 聚合在该 topic 内稳定分区 |
| transaction events | `cardTransactionId` | 同一 card transaction 聚合在该 topic 内稳定分区 |

这不保证同一卡片或两个 topic 之间的全局顺序。若业务以后要求“授权通知一定先于入账通知”，需要改用共同顺序 key/顺序域、增加可比较的业务序号/状态检查，或让 consumer 按 source of truth 去抑制乱序；当前两个不同 aggregate key 不能支持这个结论。
...
Outbox、DelayJob、StatementJob 和 NotificationDelivery 的 claimer 使用：
...
当前项目把 risk 放在 account lock 前，这是正确方向；但它仍处于 `AuthorizationService.authorize(...)` 的单事务内。也就是说没有持有 account row lock，不等于没有占用 transaction/Hikari connection，容量分析要同时看 `risk.external.latency` 与 Hikari active/pending。
...
| Notification provider | 可以延迟，不可静默丢 | delivery lease、RateLimiter/Retry/CircuitBreaker、durable retry/DEAD、provider idempotency key |
| External risk | 当前 fail closed | timeout + semaphore bulkhead + CircuitBreaker + fallback metric |
| Read replica（当前未配置） | 若未来引入，读取可降级 | 切回 primary 或 cache，但资金写入仍只认 primary |
...
```text
OutboxWorker -> Kafka -> two Notification listeners -> Inbox idempotency
-> notification intent + per-channel delivery rows
-> NotificationDelivery worker -> provider -> SENT/retry/DEAD
```
...
- Bounded worker pool。
- Kafka partition by `authorizationId` / `cardTransactionId` within each current topic。
...
```text
短期 backlog 不破坏主交易 correctness，但会扩大 eventual consistency lag。当前要分别给 Outbox oldest pending age、两个 notification consumer lag、DLT，以及 `notification_deliveries` oldest PENDING/DEAD 定 SLO；只看 Kafka lag 会漏掉 provider 侧积压。
```
...
```text
CPU/memory 只是底层指标。授权系统还要看 p99、DB lock wait、Hikari pending、external risk latency/bulkhead rejection、Outbox backlog、Kafka lag、NotificationDelivery backlog/provider latency、Redis fail-open、GC pause 和 decline reason distribution。
```
