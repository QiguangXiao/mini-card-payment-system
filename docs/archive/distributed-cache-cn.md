...
> **归档对齐说明（2026-07）**：本文已按当前三类 Redis 用法全文校准：statement L1/L2 cache、
> 按卡的 Risk velocity sliding window，以及 `POST /api/authorizations` 按调用方 IP 的 token bucket。
> `card_risk_features` 历史画像已删除；现行 Risk 只有实时 velocity、本地规则和 external risk。
...
相关代码:
- `statement/application/read/StatementReadService.java`(Caffeine L1 + Redis L2、CAS/墓碑、重建锁)
...
- `infrastructure/web/ratelimit/RedisTokenBucketRateLimiter.java`(API 入口令牌桶)
- `infrastructure/web/ratelimit/AuthorizationRateLimitInterceptor.java`(429 + `Retry-After` 出口)
...
现在项目保留一个窄范围的 `statement` GET cache:`StatementReadService` 直接使用 Caffeine L1 +
Redis L2 + cache-aside + `statements.version`/Lua CAS/墓碑 + after-commit eviction，并用默认开启的
Redis rebuild lock 防跨 pod 热 key 击穿。`card` snapshot cache 仍然删除,原因值得讲清楚——
...
当前配置是 L1 30s/1000 entries、L2 5m + 0~30s jitter、墓碑 10s、重建锁 2s；
跨 pod L1 Pub/Sub 广播已实现但默认关闭。
...
每张卡一个 ZSET,key = `risk:velocity:<cardId>`（代码未使用 Redis Cluster hash tag 花括号）,member =
每次尝试的唯一标识,score = 尝试发生的毫秒时间戳。默认窗口 60s、最多 3 次。
...
### 2.4 第二套现行限流：API 入口 Redis token bucket

Risk velocity 是**业务风控信号**；API rate limit 是**系统保护闸门**，不能混为一个阈值：

| 维度 | Risk velocity | API token bucket |
|---|---|---|
| key | card id | `request.getRemoteAddr()`；生产应换 authenticated client id |
| 位置 | authorization service 风控阶段 | MVC `preHandle`，body 反序列化和 transaction 之前 |
| 算法 | ZSET sliding-window log | HASH `{tokens, ts}` + lazy refill |
| 默认阈值 | 60s 内最多 3 次 | capacity 20，refill 10/s |
| 超限结果 | 风控 decline | HTTP 429 + 向上取整且至少 1s 的 `Retry-After` |
| Redis 故障 | degraded fail-open + metric | fail-open + `api.ratelimit.redis.unavailable` |

token bucket 的 Lua 原子完成“读状态 → 计算 refill → 扣 1 token → 写状态 → 续 TTL”。新/过期桶从满桶
开始，所以允许 capacity 大小的 burst；TTL 是“装满所需秒数 × 2，且至少 60s”，空闲桶自然清理。

多 pod 的 JVM clock 可能略有偏差，脚本使用 `effective_now = max(client_now, stored_ts)`，保证 Redis 中
`ts` 不倒退。若慢时钟把 `ts` 写回过去，快时钟 pod 下次会重复计算同一段 elapsed time，凭空多补 token，
全局限流被放宽。`RedisTokenBucketRateLimiterIT` 用真 Redis 覆盖并发、refill、TTL 和时钟回退场景；这也是
为什么限流 Lua 不能只靠 mock 接缝测试。

客户端 key 当前只信容器解析后的 `remoteAddr`，不直接读取可伪造的 `X-Forwarded-For`；反向代理环境应由
网关或 Tomcat `RemoteIpValve` 按 trusted proxies 解析真实地址。这个边界比“取一个 header”更有 interview
价值，因为它把限流正确性和 proxy trust model 联系起来。
...
本项目两种都用，但语义不同：Risk 选 **sliding window log**，因为要“过去 N 秒发生几次”的行为信号；
API 入口选 **token bucket**，因为要允许可控 burst、压住持续洪峰并返回是否通行。
...
- **应用内单机**(项目的 Resilience4j `RateLimiter`):当前用于每 pod 的 Notification provider 出站 quota，
  **不跨实例**；它不承担 API 全局限流。API 入口由 Redis token bucket 跨实例共享状态。
...
- **“先更 DB 再失效”**仍有 read-old/write-late race；delayed double-delete 只能用经验延时降低概率，
  无法证明覆盖所有慢 reader。
- 本项目在 repayment 后用 **after-commit eviction** 确保 DB 先提交，再以 `statements.version` 写 tombstone；
  读回填走 Lua CAS。墓碑保留版本地板，避免 DELETE 后迟到旧值面对空 key 又成功写回。
...
- 读写模式 + **一致性**(删 vs 更、after-commit、version/CAS/tombstone、崩溃窗口)。
- **三大问题**(穿透/击穿/雪崩)+ 解法。
...
A:更新缓存在并发下可能把旧计算结果写进去;失效让下次读回源最新值,更简单。写后失效要放在
**transaction commit 之后**，否则提交前的 reader 会回填旧值。当前项目不靠延迟双删猜 reader 最慢多久，
而是写 `statements.version` 墓碑作为版本地板，旧 reader 的 CAS 回填被拒。它仍不是强一致：commit 后、
afterCommit 前宕机或 Redis 写失败时只能由 TTL 自愈；若 stale SLO 更严，要把失效意图写入同事务 Outbox。
