...
> **归档对齐说明（2026-07）**：文中作为对比方案出现的 `statement.closed` → `statement-events` → `StatementNotificationListener` Kafka 通知链现已移除；本文的主角——Redis Pub/Sub 跨 pod L1 失效广播——仍在现行代码中（默认关闭）。文中相关段落已按现行架构清理，其余内容保持原样；现行缓存设计以 [caching-and-rate-limiting-cn.md](../caching-and-rate-limiting-cn.md) 为准。
...
还款 / 关账 (同一个业务事实)
   ...
   │
   └─(领域事件，该链路 2026-07 已移除)──► Outbox ──► Kafka ──► notification listener  [at-least-once, 幂等]
                                                     （消费者组，一个 pod 处理一次）
```

- **Notification**（该链路 2026-07 已移除，此处仅作两种通道的对比方案保留）：`statement.closed` 等
  领域事件 → Outbox → Kafka → notification listener（消费者组 + Inbox 幂等），
  与现行 Authorization/CardTransaction notification listener 同构。
