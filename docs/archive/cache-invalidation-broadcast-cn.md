...
> **归档对齐说明（2026-07）**：本文已按当前 `StatementReadService`、`StatementReadCacheBroadcastConfiguration`
> 和 `statement.read-cache.*` 全文校准。当前 cache name 是 `statement-read-model-v3`，L2 版本来自
> `statements.version`；Redis Pub/Sub 跨 pod L1 失效广播已经实现但默认关闭。历史上的
> `statement.closed` 通知链已经删除，下文只把它作为 transport 语义对照，不把它描述成现行流程。
> 现行统一说明见 [caching-and-rate-limiting-cn.md](../caching-and-rate-limiting-cn.md)。

本文档回答两个问题，并给出 statement read cache 的失效方案：

1. 缓存失效广播该用 **Redis Pub/Sub** 还是 **Kafka**？
2. 为什么 **L1 缓存失效**适合 Redis Pub/Sub，而 Authorization/CardTransaction 的
   **Notification** 适合 Outbox → Kafka → Inbox？

结论先行：**两类动作不要因为都叫 publish/subscribe 就共用 transport。L1 失效广播用 Redis Pub/Sub；
现行 Authorization/CardTransaction Notification 用 Outbox → Kafka → Inbox。前者允许丢失并由 TTL 自愈，
后者要求 durable、at-least-once 与 consumer-side idempotency。**

---

## 1. 核心对比：缓存失效广播 vs Notification

| 维度 | L1 缓存失效广播 | Authorization/CardTransaction Notification |
| --- | --- | --- |
...
**相似的“广播/通知”名字不代表相同投递语义。** transport 要按是否允许丢失、是否要求重放和消费模型选择。
...
```text
还款更新 statement
  └─ after-commit → L1 invalidate + L2 tombstone + Redis PUBLISH → 所有 pod 清 L1
                                                                  [best-effort, ms]

授权批准/拒绝、交易入账
  └─ 同事务 Outbox → Kafka → Notification listener + Inbox → intent + deliveries
                                                            [at-least-once, 幂等]
```

- **Notification**：当前只有 Authorization/CardTransaction 事件进入 Kafka 通知链；statement/repayment
  通知事件已删除。Kafka consumer group 做竞争消费，Inbox 与业务唯一键负责 duplicate delivery。
- **L1 evict**：`StatementReadService.evictAfterCommit(Statement)` 在 commit 后清本 pod L1、向 L2 写
  version tombstone，再 PUBLISH statement id；订阅者只清各自 Caffeine L1。

### 1.4 为什么不用 Redis Stream（Pub/Sub vs Stream vs Kafka 三选一）

常见追问：Redis 也有 Stream，失效广播为什么不用 Stream？因为 **Stream 的原生模型是 consumer group
竞争消费，而缓存失效要的是 fan-out 广播**——方向正好相反。三者一张表：

| | Redis Pub/Sub | Redis Stream | Kafka |
| --- | --- | --- | --- |
| 模型 | **fan-out 广播** | log + consumer group（竞争消费） | log + 分区 + consumer group |
| 投递 | at-most-once，无持久化 | at-least-once，持久化可重放 | at-least-once，持久化可重放，分区内强有序 |
| 离线消费者 | 收不到（丢） | 重连可补（XREADGROUP + PEL） | 重连可补 |
| 额外簿记 | 无 | XACK / PEL / XAUTOCLAIM / MAXLEN trim | broker / offset |
| 对口场景 | **缓存失效广播**、在线推送 | 单 Redis 内的轻量 durable 队列/事件流 | 跨服务事件总线、高吞吐、长保留 |
| 运维成本 | 近零 | 中 | 高 |

对缓存失效这件事具体推演：

- 要 **fan-out 给所有 pod** → Pub/Sub 原生支持；用 Stream 得给每个 pod 一个独立 consumer group 才能都
  收到，等于把"§1.1 Kafka 唯一 group 膨胀"那套问题原样搬到 Redis。
- 消息**幂等 + 自愈**（只是"失效某 id"，且有 L1 TTL 兜底）→ **容忍丢失**，不需要 Stream 的持久化/重放。
- Stream 的成本（trim、每 pod consumer group 的 offset/PEL、死消费者 XAUTOCLAIM、保留条目内存）全是为了
  缩小一个 `local-ttl` 已经兜住的窗口，**不划算**。

什么时候该选 Stream？需要"单 Redis 内的轻量 durable 队列/事件流"——要持久化、消费确认、重放，但还没到
上 Kafka 的规模。本项目那一格已被 **Outbox → Kafka** 占据（跨服务、可重放、长保留），所以 Stream 在这里
没有不可替代的位置。**面试要点：能讲清这张三选一矩阵即可，不必为了用而用 Stream。**
...
RepaymentService.applyRepayment (commit)
  └─ StatementReadService.evictAfterCommit(statement)  // 捕获 statement.id/version，after-commit 执行
       └─ evict(id, version):
            1. localCache.invalidate(id)               // 清本 pod L1
            2. writeViaCas(key, version, "version|")    // L2 写墓碑版本地板，不做 DELETE
...
4. **失效顺序**：先写 L2 墓碑再 PUBLISH。订阅者收到广播后重读时会把墓碑当 miss 并回源，不会命中旧 L2。
5. **频道名带 cache 版本号**（当前 `mini-card:cache-evict:statement-read-model-v3`），与 L2 的
   `CACHE_NAME` 对齐；换 read
   model schema 时一起换。
...
再做选型对比，最后给出本仓库采用的实现。约定：`version = statements.version`。它是 DB 中的单调版本，
在改变 statement read model 的同一个 transaction 内递增；不能再用 `paidAmount` 推导，因为金额不变时
due date、状态或展示字段也可能变化。
...
| 写侧知识/耦合 | 只需 `statements.version`，零 read model 知识 | 写侧要拼完整 read model |
...
- **版本**：`StatementReadModel.version()` 直接来自 `statements.version`。还款 transaction 推进 domain
  version，evict 侧使用提交后的 `statement.version()` 写墓碑，读写两侧共享同一个 DB source-of-truth 版本。
- **L2 信封格式**（当前 cache name 为 `statement-read-model-v3`）：`<version>|<payload>`
...
### 3.6 当前测试边界：statement CAS 尚缺真 Redis 集成测试
...
因此当前应准确描述为：`StatementReadServiceTest` 已覆盖 Java 接缝，但 statement CAS/tombstone 仍没有像
`RedisTokenBucketRateLimiterIT` 那样的真 Redis 集成测试。推荐用 Testcontainers 增加迟到写被拒、同版本真值
替换墓碑、乱序 evict 保持高地板等场景；在补齐前，不能把 mock 测试等同于 Redis/Lua 并发语义验证。
...
| cache name 版本号（v3） | version 语义/value contract 演进安全，旧 key 自然过期 | `CACHE_NAME` |
| Redis rebuild lock | 防跨 pod 热 key 击穿；失败时 fail-open 回源 | `rebuildWithSingleFlight` |
...
4. **version 推进完整性**：现在已有专用 `statements.version`，但未来任何会改变 read model 的写路径都必须
   在同一 DB transaction 内推进它；漏增 version 会把不同内容伪装成同版本。
5. **跨 pod 冷 key thundering herd**：当前已用 `SET NX PX` rebuild lock（默认开启）做 best-effort
   distributed single-flight；winner 之外的请求最多等待 `5 × 20ms`，超时或 Redis 故障后仍会 fail-open 回源，
   所以极端故障下仍可能有多个 pod 同时打 DB，这是有界降级而不是正确性问题。
...
（快、简单，崩溃窗口由 TTL 兜底，账单容忍秒级 stale）；要"崩溃也不丢失效"再升级到 outbox-driven。
当前 Outbox publisher 只路由 Authorization/CardTransaction 事件；若采用此方案，还要显式增加 cache-invalidation
事件路由和幂等 relay，不能借用已删除的 `statement.closed` 通知事件。
...
- 三大经典问题词汇 + 对策：穿透（按威胁模型暂不做 negative cache）、击穿（JVM 单飞 + Redis rebuild lock）、
  雪崩（jitter）；
...
**结论：不要无条件往缓存里加功能。** 当前跨 pod rebuild lock 已实现；outbox-driven 失效和 negative cache
只有在 stale SLO 或不存在 key 攻击成为真实问题时才值得增加。当前最明确的缺口是**测试严谨度**，
不是缓存功能：**用 Redis Testcontainers 把那段 Lua CAS 在真环境钉死**（见 3.6）。它既补上唯一的
