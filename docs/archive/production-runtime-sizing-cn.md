# 生产运行时容量规划（Production Runtime Sizing）

> **归档对齐说明（2026-07）**：本文已逐段对齐当前代码和 `application.yml`。现行进程显式配置为 Tomcat `80/10`、Hikari `10/5`、五组 worker pool（各 `4 + queue 100`）、五组 scheduler pool（合计 8 个调度线程），Kafka 只有两个通知 listener container（各 `concurrency=2`）。历史 Risk/Ledger projection consumer、旧 `max-per-run=100` 和“Tomcat/Hikari 未显式配置”等说法均已删除。小/中/大规格仍是容量规划示例，不代表仓库现状，更不是 SLA；现行主题文档以 [traffic-rate-limiting-and-capacity-cn.md](../traffic-rate-limiting-and-capacity-cn.md) 为准。
...
| 配置面 | 当前项目状态 | 生产含义 |
| --- | --- | --- |
| Tomcat request threads | `max=80`, `min-spare=10`, `max-connections=2048`, `accept-count=50`, connection timeout `3s`, keep-alive timeout `30s` | 80 是单进程 HTTP worker 上限，不是 TPS；`accept-count` 只在连接数达到 `max-connections` 后控制 OS accept queue，不能当成应用请求队列 |
| Hikari DB pool | `maximum-pool-size=10`, `minimum-idle=5`, connection timeout `1000ms`, validation timeout `500ms` | 所有 HTTP、Kafka 和后台 DB 工作共享这个 10 连接池；生产必须按实例数计算总连接并给 migration/admin 留余量 |
...
| Notification delivery scheduler | `notificationDeliveryTaskScheduler = 2` | dispatch/recover 调度线程；真正调 provider 在专用 worker pool |
| Outbox worker | `worker-pool-size=4`, `queue=100`, `batch-size=50` | Kafka publish 并发有限、有 queue 背压 |
| DelayJob worker | `worker-pool-size=4`, `queue=100`, `max-per-run=16`；`auto-repayment-worker-pool-size=4` 专用池 | 授权过期等纯 DB job 与调外部银行的自动扣款分池；16 是一次混合 claim 上限，不是每个池各 16 |
...
| Notification delivery worker | `worker-pool-size=4`, `queue=100`, `batch-size=40` | Inbox 只创建 durable delivery intent；provider I/O 在该池执行，lease 为 30s，durable max attempts 为 8 |
| Kafka topics | 每个主要 topic `partitions=3`, local replica=1 | 本地演示 consumer concurrency；生产要提高 replicas 和 partitions |
| Kafka listener concurrency | authorization notification=2、transaction notification=2 | 两个 listener container 属于同一 group，但订阅不同 topic；单进程最多 4 个 consumer threads，不再存在 Risk/Ledger consumer |
| Actuator | 暴露 `health/info/metrics`，不暴露高风险 endpoint | 适合生产基础监控；heapdump/env 等不应公开 |

不要把这些数字机械相加成“系统并发”。所有池都创建时，进程最多有 80 个 Tomcat worker、4 个 Kafka consumer、20 个业务 worker 和 8 个 scheduler thread，但 DB 路径仍共同竞争 Hikari 的 10 个连接；通知 provider、external risk、bank debit 又各有独立 timeout、bulkhead/rate limiter 或 circuit breaker。容量要按资源图找最窄的共享依赖，而不是按线程总数估 TPS。

> [!IMPORTANT]
> 本文曾在 2026-06-27 记录“当时 `localhost:8080` 没有应用进程”，本轮只做静态文档对齐，没有重新采集 live runtime。可保留的 2026-06-21 历史摘要是：IDEA 启动时 Java 21.0.11、G1GC、initial heap 约 256 MiB、max heap 约 4 GiB、一次授权后 live threads 约 97；原始 `build/diagnostics` 文件当前不在工作区。它既不是当前快照，也不是生产配置。
...
## 3. 当前基线与生产 override 示例

下面先用一段可部署的 override 展示所有关键配置面。示例把 Tomcat/Hikari 提高到 `160/20`，因此不是当前仓库值；它只能作为 load test 起点。若直接运行当前 `application.yml`，应以第 1 节的 `80/10` 为准。
...
| Notification worker | 2-4, queue 100 | provider 慢时由 durable delivery + 有界池吸收，不阻塞 Kafka listener |
...
delay-jobs.scheduler:
  # 示例取两个 8 线程池总 worker 的约 2 倍；仍要按 job type 分布和 lease 实测。
  max-per-run: 32
  worker-pool-size: 8
  worker-queue-capacity: 200
  auto-repayment-worker-pool-size: 8
  auto-repayment-worker-queue-capacity: 200
...
notification.delivery:
  # worker 增大不会绕过每渠道 RateLimiter；quota 也必须按 provider 全局预算重算。
  batch-size: 80
  worker-pool-size: 8
  worker-queue-capacity: 200
```
...
> [!TIP]
> Medium 阶段最有价值的架构动作通常不是“把 Tomcat 开到 500”，而是把 API service 和 batch/consumer-heavy service 分开部署。当前 Outbox、DelayJob、StatementJob 和 NotificationDelivery scheduler 已有 enable 开关；Kafka listener 尚没有独立的业务开关，真正拆角色时要补显式 listener enable/auto-startup 配置，不能只靠“约定这个 Pod 不消费”。这样 statement 月末出账不会抢授权 API 的 CPU、DB connections 和 worker queue。
...
delay-jobs.scheduler:
  # 混合 claim 上限；不要用巨大 batch 把 durable backlog 提前搬进 JVM queue。
  max-per-run: 48
  worker-pool-size: 12
  worker-queue-capacity: 300
  auto-repayment-worker-pool-size: 12
  auto-repayment-worker-queue-capacity: 300
...
- authorization event 以 `authorizationId`、transaction event 以 `cardTransactionId` 作为 Kafka key，保证同一聚合在各自 topic 内稳定分区；它们不提供同一卡片维度或跨 topic 的全局顺序。
- 两个 listener container 各自最多 2 个 consumer thread；把单进程 concurrency 加到 4 以上，也会受每个 topic 3 partitions 限制。
- Consumer Inbox 仍然必须存在，因为 Kafka retry/duplicate delivery 无法和 MySQL 本地事务合成一个原子提交；Inbox 去重后只创建 notification delivery intent，provider 调用在后续 durable worker 中发生。
...
1. 保持 external risk 在 account row lock 之前，避免 request A 持有账户锁等待外部 I/O；但当前单事务仍持有 Hikari connection 和 authorization winner row lock，不能误讲成“完全事务外”。
...
5. Tomcat busy 高但 Hikari pending 不高时，看 external risk latency、Redis timeout、JSON serialization 和 CPU。
6. Outbox backlog 高时，调 Outbox worker/Kafka，而不是把 Kafka publish 放回主事务。
...
- StatementJob PENDING/PROCESSING/DONE/DEAD count。
- NotificationDelivery PENDING/PROCESSING/SENT/DEAD count、oldest pending age、provider latency/rejection。
...
不要只压一个 `/actuator/health`。本项目至少要压这些真实业务路径：
...
5. 压完后检查 Outbox/Inbox/DelayJob/StatementJob/NotificationDelivery 是否有 DEAD 或长期 PROCESSING。
```
...
> 当前项目已经显式配置 Tomcat `max=80/min-spare=10` 和 Hikari `max=10/min-idle=5`，但没有提交生产容器的 `JAVA_TOOL_OPTIONS`。后台有 Outbox 1、DelayJob 2、BillingCycle 1、StatementJob 2、NotificationDelivery 2 个 scheduler threads；Outbox、普通 DelayJob、自动扣款、StatementJob、NotificationDelivery 五个 worker pool 默认各 4 线程、queue 100。生产部署仍要把实例数、总 DB connections、heap/native memory 和各类 worker 职责写进 profile，并用 Actuator、Kafka lag、MySQL 指标和 `jcmd` 验证。
