...
> **归档对齐说明（2026-07）**：文中 `card_risk_features` 的读路径、延迟与容量条目已随 historical risk projection 移除而失效，其余 sizing 方法论仍适用。文中相关段落已按现行架构清理，其余内容保持原样；现行容量文档以 [traffic-rate-limiting-and-capacity-cn.md](../traffic-rate-limiting-and-capacity-cn.md) 为准。
