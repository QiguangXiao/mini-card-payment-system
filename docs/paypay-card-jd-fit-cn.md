...
| Kafka 重复投递 | Inbox / unique source event | `ConsumerInboxRepository`, notification/risk listeners | consumer side effect 幂等 |
...
| Notification/Risk 异步 | Kafka consumers | 下游失败不阻塞主交易 |
...
| Kafka consumer lag | 下游 Notification/Risk 是否追上 |
...
> 真正额度变化时用 `SELECT ... FOR UPDATE` 锁 account row。Kafka publish、notification、risk projection
...
> 如果状态变了但没有 Outbox event，下游 Notification/Risk 会丢业务事实。
...
> 因为同一事件可能被 Notification、Risk 各自处理一次。
...
> 项目里的 Risk feature 更像异步投影；production Ledger entry 则通常属于更强的 accounting 边界。
...
> 更适合先拆 Notification、Risk projection 这类异步下游。
...
> 更合理的演进是先拆 Notification、Risk projection 这类异步下游。
...
> Notification/Risk 已足够展示 Kafka/Inbox 幂等，因此删除重复 projection，把 Ledger 留在概念和生产演进讨论中。
...
> 同一个 event 对 Notification、Risk 是两次不同消费。
...
> Notification 创建通知，Risk 更新 feature projection。
...
> 需要判断业务影响：notification 延迟通常可容忍，risk projection 落后可能影响后续决策数据。
...
> 对 Kafka duplicate，重复投递同 eventId，确认 Notification/Risk side effect 幂等。
...
> 再查 authorization、credit_account、Outbox event、Kafka consumer、notification/risk projection。
