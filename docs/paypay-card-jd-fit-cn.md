...
> Redis 只缓存可重建 snapshot，例如 statement read model 和 card snapshot。
> Card snapshot 参与卡状态判断，所以 TTL 要短，并且未来 block/unblock API 必须 after-commit evict 或发布 invalidation event。
...
> 账单生成必须定义 period cutoff，并在 transaction 内固定本次 statement items。
...
> `statement`、`statement_items` 和交易标记应在同一个 transaction boundary 内完成。
...
> 显式 `SnapshotCache` 比散落的 `@Cacheable` 更容易解释和排障。
...
> 删除 Redis 后，本地 L1 仍可能返回旧 card snapshot。
> 需要 L1 invalidation、短 TTL 或高风险路径绕过 L1。
...
> Statement read model 从 statement/statement_items/repayment 状态回源。
