...
```text
Statement: 创建 CLOSED
StatementItem: 快照 posted transactions
CardTransaction: 写入 statementId
DelayJob: 创建 AUTO_REPAYMENT
OutboxEvent: statement.closed
```
...
- `statement_items` 是出账时的快照，不是每次查询时临时 SUM。
- 自动扣款是 future business action，所以用 DelayJob，不用 Outbox。

30 秒回答：

> 账单生成不是简单查询汇总，而是创建一个可审计的账单快照。实现上先锁账户行作为 concurrency gate，再查找同周期是否已有 statement。如果没有，就锁住本周期未出账的 posted transactions，创建 statement 和 statement items，并把这些交易标记为已归入该 statement。这样可以防止同一笔交易进入两张账单，也避免账单金额之后被历史交易变化影响。
...
| Statement | statement、statement items、transaction assignment、auto repayment DelayJob、Outbox event | PDF 生成、通知发送 |
...
| 机制 | 本质 | 例子 |
| --- | --- | --- |
| Outbox | 可靠发布已经发生的业务事实 | `authorization.approved`, `statement.closed` |
...
### Q6：为什么账单要保存 `statement_items` 快照？

答：

> 账单是对某个 billing cycle 的正式结果，应该稳定、可审计。不能每次查询时临时 SUM 当前交易表，因为后续退款、调整、交易字段修正可能改变查询结果。`statement_items` 保存出账时的交易快照，让用户看到的账单明细稳定。
...
> 它是学习项目，不是完整生产信用卡核心。真实系统还需要账户持有人、权限认证、审计日志、ledger、settlement/reconciliation、refund/dispute、监管报表、监控告警、迁移工具、灰度发布、密钥管理和更严格的安全控制。但当前项目已经覆盖后端interview最常问的交易状态、一致性、幂等、锁、异步可靠性和失败恢复。
...
12. 解释 DDD 在项目里带来了什么，哪里没有强行套 DDD。
13. 解释 MyBatis 相比 JPA 在这个项目里的优势。
14. 解释金额为什么用 `BigDecimal` 和 `DECIMAL(19,2)`。
15. 说出这个学习项目离真实生产系统还缺哪些东西。
