...
| Cache 边界 | `SnapshotCache`, `TwoLevelSnapshotCache` | 只缓存可重建 snapshot，不缓存额度 |
| Redis timeout | `spring.data.redis.timeout=1s` | cache 不应拖垮主请求 |
...
| Redis/cache | timeout, miss storm, stale data | `TwoLevelSnapshotCache` |
...
当前项目已有单 JVM per-key single-flight：
...
Card snapshot 参与 authorization 决策，比 statement GET 风险高。
