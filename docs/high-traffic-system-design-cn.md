...
| 历史 cache 边界 | `docs/cache-snapshot-design-cn.md` | 已删除的 two-level cache-aside 设计仍可学习“不缓存额度” |
...
| Redis/cache | timeout, limiter degradation, historical cache miss storm/stale data | `RedisRiskVelocityCounter`, historical `TwoLevelSnapshotCache` docs |
...
```text
Thread 1 enters TwoLevelSnapshotCache.get(stmt-001)
  -> creates local lock for stmt-001
  -> reads Redis miss
  -> loader calls StatementService.get(stmt-001)
  -> writes L1 and Redis
...
- `TwoLevelSnapshotCache.localLocks`
- `synchronized(lock)`
...
#### 剧本 8：Card blocked 后，某个 pod 的 L1 仍是 ACTIVE
...
```text
Pod A CachedCardRepository reads local L1 ACTIVE
...
- `card-snapshot-v1.local-ttl = 10s`。
- `remote-ttl = 1m`。
- 文档明确 Card snapshot 风险高于 statement read model。
...
无论当前 Redis velocity 还是历史 snapshot cache，都不适合：
...
历史 snapshot cache 曾经有单 JVM per-key single-flight：

```text
localLocks.computeIfAbsent(key, ...)
synchronized(lock)
```
