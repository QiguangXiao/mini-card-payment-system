...
| [`hardcore-interview-qa-cn.md`](hardcore-interview-qa-cn.md) | ✅ | **全类型问答种子库（2026-07-21 新建）**：覆盖问答总库之外的面试维度——专业基础深水区（D1：四方模型 + authorization/posting/statement/repayment 领域核心一问；D2–D6：HTTP·TCP/Linux 排障/IO 模型与 Redis·Netty/Spring 事务失效/分布式理论 CAP·共识·fencing）、通用后端工程深水区（E1–E10：索引/MVCC/分布式事务/分库分表/JVM 排障/JMM 与虚拟线程/API 演进/安全/大表在线 DDL/MyBatis SQL 注入）、实务编码（C1–C5：LRU/令牌桶/阻塞队列/CompletableFuture 编排/SQL 窗口函数）、生产实战（P1–P6：偶发尖刺系统化排查/线上 debug 工具箱与 Arthas/事故响应前 15 分钟与复盘/数据订正/真实测试策略/发布纪律与 feature flag）、工程日常（W1–W8：需求到上线全流程/技术选型框架与 ADR/git 与小 PR/设计文档/估算与低估成因/效率=反馈回路×专注/AI 提效与新问题/技术债管理）、独立系统设计题（S1–S3：积分发放/清分对账/遗留迁移，S3 附六个深挖专题 S3-a〜f：数据管道/shadow 与 diff 引擎/切流回切/双写失败矩阵/长尾状态/组织节奏）、行为面试 STAR（B1–B5）、团队/心理/人际（T1–T6，含 AI 协作）、国际化/动机/日本职场（G1–G5）、逆质问（R1–R8）；重要题带追问追答，编号按类预留后续拓展 |
