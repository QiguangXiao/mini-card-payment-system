...
5. `StatementLine.snapshot(...)` 创建账单 line。
   ...
   `statement_lines` 保存明细快照：哪笔交易、金额、币种、posted 时间、展示用交易号等。
...
| `Authorization.post(...)` | `AuthorizationPostedDomainEvent` | `authorization.posted` | 当前无消费者（Notification listener 刻意跳过 posted：它是发卡方内部 hold 生命周期，用户可见入账通知走 `card_transaction.posted`） |
...
- 创建期失败：不 rollback authorization/posting 主事务，Kafka listener retry / DLT 处理。
